...
(function () {
  ...
  function ann_skin_post_header_init() {
    var header = jQuery(
      ".post_header_wrap_style_style-1.with_featured_image, .post_header_wrap_style_style-2.with_featured_image"
    );
    ...
    if (
      jQuery(header).find(
        ".post_featured.with_thumb.post_featured_bg.with_video"
      ).length > 0
    ) {
      return;
    }
  ...
  if (
    ...
    TRX_ADDONS_STORAGE["mouse_helper"] > 0 &&
    TRX_ADDONS_STORAGE["mouse_helper_in_swiper_slider"] > 0
  ) {
    if (
      jQuery("body").find(".slider_container.slider_swiper:not(.go)").length > 0
    ) {
      jQuery("body")
        .find(".slider_container.slider_swiper:not(.go)")
        .each(function () {
          jQuery(this).addClass("go");
          var check_helper = jQuery(this)
          ...
          if (!check_helper) {
            ...
            jQuery(this).attr(
              ...
              "<div class=custom-helper><span></span></div>"
            );
  ...
  function skin_blogger_controls_title() {
    if (jQuery("body").find(".sc_slider_controls:not(.go)").length > 0) {
      jQuery("body")
        .find(".sc_slider_controls:not(.go)")
        .each(function () {
  ...
  if (document.getElementById("bannerSVG") !== null) {
    ...
    var resizeHandler = function () {
      ...
      (_[0].textContent = ""), (s = w((o.viewBox.baseVal.width - 10) / r));
    };

    var updateTextAnimation = function (e) {
      d || (d = 0.001 * e), requestAnimationFrame(updateTextAnimation);
      for (
        var n = 0.001 * e - d,
          t = (function (e) {
            return e < 0.5
  ...
  ann_add_filter(
    ...
    function ($param, $init) {
      if (
        $init.parents(".slider_width_auto").length > 0 &&
        ($init.parents(".sc_portfolio_extra").length > 0 ||
          $init.parents(".sc_portfolio_default").length > 0)
      ) {
  ...
  function skin_blogger_toggle_filter_add_element() {
    if (jQuery(".content").find(".sc_style_toggle").length > 0) {
      jQuery(" .sc_style_toggle").each(function () {
        ...
        jQuery(this)
          .find(".sc_item_filters")
          .append(
            '<div class="title_wrap"><span class="toggle_title"></span><span class="active_tab"></span></div>'
          );
        jQuery(this)
          .find(".sc_item_filters .sc_item_filters_tabs")
          .wrap('<div class="list_wrap"></div>');
        jQuery(this).find(".sc_item_filters .toggle_title").text(toggle_title);

        jQuery(this)
          .find("span.active_tab")
          .mouseenter(function () {
            jQuery(this)
              .parents(".sc_item_filters")
              .find(".sc_item_filters_tabs")
              .addClass("active");
          });
        jQuery(this)
          .find(".sc_blogger_filters")
          .mouseleave(function () {
    ...
    if (jQuery(".content").find(".sc_style_toggle").length > 0) {
      jQuery(" .sc_style_toggle").each(function () {
        var text = jQuery(this)
          .find(
            ".sc_item_filters .sc_item_filters_tabs .sc_item_filters_tabs_active a"
          )
          .text();
  ...
  function skin_add_custom_width_filter_list() {
    if (jQuery(".content").find(".sc_style_toggle").length > 0) {
      jQuery(".sc_style_toggle .sc_item_filters_wrap .sc_item_filters").each(
        function () {
          var check_element = jQuery(this).find(".list_wrap");
          var width = jQuery(this).find(".title_wrap").outerWidth();
          jQuery(check_element).css(
            "width",
            "calc(100% - " + (width + 10) + "px)"
          );
        }
      );
    }
  ...
  function mousemove_for_portfolio_small() {
    if (jQuery(window).width() > 1279) {
      if (
        jQuery(".content").find(".sc_blogger_lay_portfolio_grid_grid_style_7")
          .length > 0
      ) {
        jQuery(
          ".sc_blogger_lay_portfolio_grid_grid_style_7 .sc_blogger_item"
        ).each(function () {
  ...
  function add_portfolio_image_wrap() {
    if (
      ...
      var $item = jQuery(
        ".sc_blogger_lay_portfolio_grid_grid_style_7 .sc_blogger_item:not(.inited_item)"
      );
      $item.each(function () {
    ...
    if (
      jQuery(".content").find(".sc_blogger_lay_portfolio_grid_grid_style_9")
        .length > 0
    ) {
      var $item = jQuery(
        ".sc_blogger_lay_portfolio_grid_grid_style_9 .sc_blogger_item:not(.inited_item)"
      );
      $item.each(function () {
        jQuery(this).addClass("inited_item");
        jQuery(this).find(".bg_in").wrap('<div class="bg_in_wrap"></div>');
  ...
  function mousemove_for_portfolio_large() {
    if (jQuery(window).width() > 1279) {
      if (
        ...
        jQuery(
          ".sc_blogger_lay_portfolio_grid_grid_style_9 .sc_blogger_item"
        ).each(function () {
          let $Target = jQuery(this);
          let $TargetInner = $Target.find(".post_info_bl");
          $Target
  ...
  function background_image_portfolio_stretch() {
    if (jQuery(".content").find(".sc_blogger_portmodern").length > 0) {
      jQuery(".sc_blogger_portmodern .sc_blogger_item .sc_blogger_item_title")
        ...
        .on("mouseleave", function () {
          ...
          jQuery(this)
            .parents(".sc_blogger_content")
            .removeClass("active_items");
        });
      if (jQuery(window).width() > 1279) {
        jQuery(
          ".sc_blogger_portmodern .sc_blogger_item:not(.inited_item)"
        ).each(function () {
          ...
          let $WrapImage = jQuery(this)
            .parents(".sc_blogger_portmodern")
            .find(".decor_image_wrap");

          if (jQuery($WrapImage).find("#" + $TargetInnerId).length <= 0) {
            jQuery($WrapImage).append(
              '<div id="' + $TargetInnerId + '" class="image_item" ></div>'
            );
            jQuery($TargetInner).on("mouseover", function () {
              let $NewTarget = jQuery(this)
                .parents(".sc_blogger_portmodern")
                .find(".decor_image_wrap");
              jQuery($NewTarget)
                .find("div#" + $TargetInnerId + "")
                .css({ "background-image": "url(" + $TargetInnerImg + ")" });
            });
          }
        });
      }
      ...
      jQuery(".sc_blogger_portmodern .sc_blogger_item").each(function () {
        ...
        jQuery(this)
          .find(".sc_blogger_item_title")
          .on("click", function () {
            let $eachTitleItem = jQuery(this)
              .parents(".sc_blogger_portmodern")
              .find(".sc_blogger_item")
              .find(".sc_blogger_item_title");
            let $WrapPreview = jQuery(this)
              .parents(".sc_blogger_portmodern")
              .find(".portfolio_preview");
            let $PreviewDescription = jQuery($WrapPreview).find(
            ...
            // Socials share
            //----------------------------------------------
            var $share_items = $PreviewDescription.find(
  ...
  function ann_skin_audio_init() {
    jQuery(
      '[class*="single_style_style-"]:not(.single_style_style-1):not(.single_style_style-3) .post_header_wrap_in_header .post_featured.with_audio.with_thumb:not(.audio_inited)'
    )
      .addClass("audio_inited")
      .each(function () {
  ...
  jQuery(document).on("action.ready_ann", function () {
    ...
    // for woo tinv
    if (
      jQuery(".woocommerce-product-gallery__wrapper > .tinv-wraper").length > 0
    ) {
    ...
    jQuery(".tribe-events-view").on(
      "beforeAjaxComplete.tribeEvents beforeAjaxSuccess.tribeEvents beforeAjaxError.tribeEvents",
      ann_tribe_events_after_ajax
    );
    ...
    jQuery(".wpcf7-submit")
      .wrap("<span class='wpcf7-submit-style'></span>")
      .after("<span class='submit-style-in'></span>");
    ...
    function textarea_auto_height(txt) {
      ...
      if (txt.val()) {
        ...
        txt.animate({ height: hiddenDiv.outerHeight() }, 800);
      }
      txt.bind("keyup", function () {
        content = txt.val();
        content = content.replace(/\n/g, "<br>");
        hiddenDiv.html(content);
        txt.animate({ height: hiddenDiv.outerHeight() }, 50);
      });
    }
            ...
            rules: [
              {
                ...
                min_length: {
                  value: 7,
                  message: ANN_STORAGE["mc4wp_msg_email_min"],
                },
                max_length: {
                  value: 60,
                  message: ANN_STORAGE["mc4wp_msg_email_max"],
                },
                mask: {
                  value:
    ...
    $document.on("action.mobile_menu_open", function (e, $menu) {
      $menu
        .find('.menu_mobile_widgets_area a[href*="mailto"]')
        .addClass("underline_anim");
      setTimeout(function () {
        $menu
          .find('.menu_mobile_widgets_area a[href*="mailto"]')
          .addClass("underline_do_hover");
      }, 600);
    });
    $document.on("action.mobile_menu_close", function (e, $menu) {
      ...
      setTimeout(function () {
        ...
        jQuery('.menu_mobile_widgets_area a[href*="mailto"]').removeClass(
          "underline_do_hover"
        );
      }, 1000);
    });
    ann_add_filter(
      "ann_filter_mobile_menu_close_timeout",
    ...
    function ann_skin_open_panel(id) {
      ...
      $panel
        .find('[class*="trx_addons_title_with_link"] > a:not(.underline_anim)')
        .addClass("underline_anim");
      setTimeout(function () {
        $panel
          .find('[class*="trx_addons_title_with_link"] > a.underline_anim')
          .addClass("underline_do_hover");
      }, 600);
    ...
    function ann_skin_close_panel() {
      ...
      setTimeout(function () {
        ...
        $panel
          .find('[class*="trx_addons_title_with_link"] > a.underline_anim')
          .removeClass("underline_do_hover");
    ...
    jQuery(".sc_layouts_panel_menu_close, .sc_layouts_panel_menu_overlay").on(
      "click",
      function (e) {
        ann_skin_close_panel();
        e.preventDefault();
  ...
  if ($search_wrap.length > 0) {
    jQuery(
      $search_wrap.each(function () {
        ...
        $search_wrap_init
          .find(".search_close")
          .add($search_wrap_init.next(".search_overlay"))
          .on("click", function (e) {
            $body.addClass("search_out_anim");
  ...
  function extra_margin_top_video_popup_button() {
    if (
      ...
      var alter_height = jQuery(
        ".single-format-video .post_header_wrap_style_style-1 .post_featured.with_video.with_thumb.post_featured_bg + .post_header"
      ).height();
      jQuery(
        ".single-format-video .post_header_wrap_style_style-1 .post_featured.with_video.with_thumb.post_featured_bg .post_video_hover_popup"
      ).css({ "margin-top": -alter_height / 2 + "px", opacity: 1 });
    }
  }
  ...
  jQuery(
    ".ann_woocommerce_filters_button,.sidebar > .sidebar_close,.sidebar_overlay"
  ).on("click", function (e) {
  ...
  jQuery(".sc_button_slide").each(function () {
    ...
    $item.off(".button-slide").on("mouseenter.button-slide", function () {
      if (
        $item.hasClass("hovered") ||
