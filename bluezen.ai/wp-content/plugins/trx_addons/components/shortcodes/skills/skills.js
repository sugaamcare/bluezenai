...
(function () {
  ...
  $document.on("action.init_trx_addons", function () {
    var $skills_items, $skills_canvas;
    ...
    function trx_addons_sc_skills_init(e, container) {
      ...
      $skills_items.each(function (idx) {
        ...
        if (
          skillsItem.hasClass("inited") ||
          !skillsItem.hasClass("trx_addons_in_viewport") ||
          skillsItem.parents("div:hidden,article:hidden").length > 0
        ) {
          return;
        }
        var init_ok = true;
        ...
        total.data(
          ...
          Math.max(
            ...
            start != parseInt(start, 10)
              ? ("" + start).length - 1 - ("" + parseInt(start, 10)).length
              : 0,
            stop != parseInt(stop, 10)
              ? ("" + stop).length - 1 - ("" + parseInt(stop, 10)).length
              : 0
          )
        );
        ...
        if (type == "bar") {
          ...
          if (window.ChartLegacy) {
            ...
            var options = trx_addons_apply_filters(
            ...
            new ChartLegacy(canvas.get(0).getContext("2d")).Doughnut(
              pieData,
              options
            );
          } else {
    ...
    function trx_addons_sc_skills_resize() {
      ...
      $skills_canvas.each(function () {
        ...
        if (
          !window.ChartLegacy ||
          canvas.parents("div:hidden,article:hidden").length > 0
        ) {
          return;
        }
