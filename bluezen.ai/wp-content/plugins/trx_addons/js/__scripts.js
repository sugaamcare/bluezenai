...
(function () {
  ...
  window.dcs === undefined &&
    (window.dcs = function () {
      console.trace();
    });
  window.dcts === undefined &&
    (window.dcts = function (name) {
      console.time(name ? name : "timer");
    });
  window.dctl === undefined &&
    (window.dctl = function (name) {
      console.timeLog(name ? name : "timer");
    });
  window.dcte === undefined &&
    (window.dcte = function (name) {
  ...
  window.trx_addons_form_validate = function (form, opt) {
    ...
    if (typeof opt.success_message_class == "undefined")
      opt.success_message_class = "trx_addons_message_box_success";
    ...
    if (error_msg !== "" && opt.error_message_show) {
      ...
      if (opt.error_message_class)
        error_message_box.addClass(opt.error_message_class);
    ...
    if (parent_val) {
      ...
      if (list_not_selected) {
        ...
        if (list_type == "select") {
          ...
          if (opt.length > 0) {
            args[0] = { key: opt.val(), value: opt.text() };
          }
        }
      }
  ...
  var _update_document_height = function (e) {
    if (
      _document_height_first_run &&
      e &&
      e.namespace == "init_hidden_elements"
    ) {
  ...
  $document.on(
    "action.init_hidden_elements action.got_ajax_response",
  ...
  window.trx_addons_adminbar_height_calc = function () {
    return trx_addons_apply_filters(
      "trx_addons_filter_adminbar_height",
      $adminbar.length === 0 ||
  ...
  var _update_fixed_rows = function () {
    ...
    $html_dom.style.setProperty(
      "--fixed-rows-height",
      _fixed_rows_height + trx_addons_adminbar_height() + "px"
    );
  };
  $document.ready(_update_fixed_rows);
  $document.on(
    "action.ready_trx_addons action.sc_layouts_row_fixed_on action.sc_layouts_row_fixed_off",
    _update_fixed_rows
  );
  ...
  window.trx_addons_fixed_rows_height = function () {
    ...
    return (
      (with_admin_bar ? trx_addons_adminbar_height() : 0) +
  ...
  window.trx_addons_document_animate_to = function (id, callback) {
    ...
    if (isNaN(id)) {
      ...
      oft = split_animation
        ? obj.offset().top
        : Math.max(0, obj.offset().top - trx_addons_fixed_rows_height());
      if (split_animation) {
    ...
    var speed = Math.min(
      ...
      Math.max(
        300,
        Math.round(
          (Math.abs((oft2 < 0 ? oft : oft2) - jQuery(window).scrollTop()) /
    ...
    if (speed > 0) {
      jQuery("body,html")
        .stop(true)
        .animate({ scrollTop: oft2 }, speed, "linear", function () {
          _window_scroll_top = $window.scrollTop();
          window.trx_addons_document_animate_to_busy = false;
          if (callback) callback(id, speed);
        });
  ...
  window.trx_addons_parse_query_string = function (qs) {
    var query = {},
      pairs = (
        qs.indexOf("?") >= 0 ? qs.substring(qs.indexOf("?") + 1) : qs
      ).split("&"),
      pair = [];
  ...
  window.trx_addons_get_embed_from_url = function (url, autoplay, mute, loop) {
    ...
    if (url.indexOf(".mp4") > 0 || trx_addons_is_local_link(url)) {
      embed =
        '<video class="trx_addons_video_video" playsinline disablepictureinpicture' +
      ...
      embed =
        '<iframe src="' +
        url +
        '" border="0" width="1280" height="720"' +
    ...
    container.find(".video_frame > iframe, iframe").each(function () {
      ...
      if (src) {
        ...
        if (src.indexOf("youtube") >= 0 || src.indexOf("vimeo") >= 0) {
          jQuery(this).attr(
            "src",
            trx_addons_add_to_url(src, { autoplay: value })
          );
        }
      }
    });
  ...
  window.trx_addons_insert_video_iframe = function ($cont, iframe_html) {
    ...
    if (
      ...
      iframe_html.indexOf("youtu") > 0 &&
      iframe_html.indexOf("autoplay=1") > 0 &&
      typeof YT != "undefined"
    ) {
      ...
      $cont.html(
        iframe_html.replace(
          /<iframe[\s]+[\s\S]+<\/iframe>/,
          '<div class="trx_addons_yt_player" id="' + id + '"></div>'
        )
      );
  ...
  window.trx_addons_browser_classes = function () {
    var userAgent = navigator.userAgent,
      ...
      classes = {
        ...
        iphone:
          trx_addons_browser_is_iphone() ||
          trx_addons_browser_is_ipad() ||
          trx_addons_browser_is_ipod(),
        ...
        ie:
          /Trident|MSIE/.test(userAgent) && (false || !!document.documentMode),
        blink: matchUserAgent("Chrome") && !!window.CSS,
        webkit: matchUserAgent("AppleWebKit"),
      };
    classes["edge"] =
      (!classes["ie"] && !!window.StyleMedia) || matchUserAgent("Edg");
    classes["chrome"] =
      !!window.chrome &&
      matchUserAgent("Chrome") &&
      !classes["edge"] &&
      !classes["opera"];
  ...
  window.trx_addons_get_basename = function (path) {
    return trx_addons_get_file_name(path).replace(
      "." + trx_addons_get_file_ext(path),
      ""
    );
  };
      ...
      jQuery
        ...
        .done(function (response) {
          if (
            ...
            typeof response.childElementCount != "undefined" &&
            response.childElementCount > 0 &&
            typeof response.children != "undefined" &&
            typeof response.children[0] != "undefined"
          ) {
  ...
  window.trx_addons_get_inline_svg = function (svg_url, view) {
    ...
    if (html === "") {
      trx_addons_fetch_url(svg_url, function (html) {
        inline_svg.push({ url: svg_url, html: html });
        if (view) {
          view.render();
        }
      });
    ...
    if (str.indexOf("[") >= 0 && str.indexOf("]") >= 0) {
      str = str.replace(
        ...
        function (match, p1, p2, p3, p4) {
          var allowed_css = trx_addons_apply_filters(
            ...
            image_css = trx_addons_apply_filters(
              "trx_addons_filter_prepare_macros_allowed_css",
              { size: { rule: "max-height", default: "1em" } },
              "image"
            ),
            icon_css = trx_addons_apply_filters(
              "trx_addons_filter_prepare_macros_allowed_css",
          ...
          if (p2 == "image") {
                  ...
                  jQuery.get(
                    ...
                    function (response) {
                      if (
                        ...
                        TRX_ADDONS_STORAGE["fetch_images"][atts["id"]] =
                          response;
                      }
                    }
                  );
                } else {
                  atts["url"] = get_url_by_size(
                    TRX_ADDONS_STORAGE["fetch_images"][atts["id"]],
                    atts["thumb"] ? "-" + atts["thumb"] : "full"
                  );
                }
              }
            }
            return atts["url"]
              ? '<img src="' +
                  ...
                  '"' +
                  (atts["id"]
                    ? ' id="trx_addons_image_' + atts["id"] + '"'
                    : "") +
                  (atts["alt"] ? ' alt="' + atts["alt"] + '"' : "") +
                  ' style="' +
                  trx_addons_get_css_from_atts(
                    atts,
                    trx_addons_object_merge(allowed_css, image_css)
                  ) +
                  (atts["css"] ? atts["css"] : "") +
                  '"' +
                  ">"
            ...
            return atts["name"]
              ? '<span class="' +
                  ...
                  trx_addons_get_css_from_atts(
                    atts,
                    trx_addons_object_merge(allowed_css, icon_css)
                  ) +
                  (atts["css"] ? atts["css"] : "") +
                  '"' +
                  "></span>"
  ...
  window.trx_addons_is_on = function (prm) {
    return (
      prm > 0 ||
      ["true", "on", "yes", "show"].indexOf(("" + prm).toLowerCase()) >= 0
    );
  };
  window.trx_addons_is_off = function (prm) {
    return (
      ...
      prm === "" ||
      prm === 0 ||
      ["false", "off", "no", "none", "hide"].indexOf(
        ("" + prm).toLowerCase()
      ) >= 0
    );
  };
  ...
  window.trx_addons_is_empty = function (prm) {
    return (
      ...
      prm === 0 ||
      prm === false ||
      (typeof prm == "object" && Object.keys(prm).length === 0)
    );
  };
  ...
  window.trx_addons_chg_class_by_prefix = function (
    classes,
    prefix,
    new_value
  ) {
  ...
  window.trx_addons_get_column_class = function (
    num,
    all,
    all_tablet,
    all_mobile
    ...
    if (all_tablet) {
      column_class +=
        " " +
        column_class_tpl.replace("$1", num).replace("$2", all_tablet) +
        "-tablet";
    }
    if (all_mobile) {
      column_class +=
        " " +
        column_class_tpl.replace("$1", num).replace("$2", all_mobile) +
        "-mobile";
    }
    return column_class;
  };
  window.trx_addons_get_responsive_classes = function (
    prefix,
    atts,
    param,
    default_value
    ...
    list = trx_addons_apply_filters(
      "trx_addons_filter_responsive_classes",
      list,
      prefix,
      atts,
      param
  ...
  window.trx_addons_wrap_chars = function (
    txt,
    before,
    after,
    before_word,
    ...
    if (unit) {
      if (unit == "vw") {
        ...
        value = Math.round(
          (value *
            (dir == "x"
              ? block.eq(0).outerWidth()
              : block.eq(0).outerHeight())) /
            100
        );
  ...
  window.trx_addons_get_css_from_atts = function (atts, allowed) {
    var css = "",
      rule = "",
      atts_with_units = trx_addons_apply_filters(
        "trx_addons_filter_atts_with_units",
        [
          "margin",
          "padding",
          "border-radius",
    ...
    for (var k in allowed) {
      if (
        allowed[k] &&
        (atts.hasOwnProperty(k) ||
          (typeof allowed[k] == "object" &&
            allowed[k].hasOwnProperty("default")))
      ) {
        ...
        if (typeof allowed[k] == "object") {
          css +=
            ...
            ":" +
            (atts[k]
              ? atts_with_units.indexOf(rule) >= 0
                ? trx_addons_prepare_css_value(atts[k])
                : atts[k]
              : atts_with_units.indexOf(rule) >= 0
              ? trx_addons_prepare_css_value(allowed[k]["default"])
              : allowed[k]["default"]) +
            ";";
        } else {
          css +=
      ...
      if (nextSource !== undefined && nextSource !== null) {
        ...
        for (
          ...
          if (desc !== undefined && desc.enumerable) {
            if (
              trx_addons_is_object(to[nextKey]) &&
              trx_addons_is_object(nextSource[nextKey])
            ) {
    ...
    if (typeof TweenMax != "undefined") {
      ...
      var interval = Math.min(
        ...
        Math.max(
          1,
          Math.round(
            (args.time * 1000) / (Math.abs(args.end - args.start) / amount)
          )
        )
      );
  ...
  var support = {
    ...
    trans_end_event_name = window.Modernizr
      ? trans_end_event_names[Modernizr.prefixed("transition")]
      : trx_addons_transition_end(),
    ...
    anima_end_event_name = window.Modernizr
      ? anima_end_event_names[Modernizr.prefixed("animation")]
      : trx_addons_animation_end();
  ...
  window.trx_addons_create_observer = function (id, obj, callback, args) {
    if (typeof window.MutationObserver !== "undefined" && obj && obj.length) {
      if (typeof trx_addons_observers[id] == "undefined") {
        var defa = { attributes: false, childList: true, subtree: true };
        if (args) {
        ...
        trx_addons_observers[id].observer.observe(
  ...
  window.trx_addons_check_mutations = function (mutations, selector, action) {
    ...
    if (
      ...
      !mutations.hasOwnProperty("length") ||
      !mutations.length
    ) {
      return rez;
    }
    ...
    for (var i = 0; i < mutations.length; i++) {
      nodes =
        action == "add" ? mutations[i].addedNodes : mutations[i].removedNodes;
      for (var n = 0; n < nodes.length; n++) {
        var $node = jQuery(nodes[n]);
        if (
          (selector.charAt(0) == "." && $node.hasClass(selector.slice(1))) ||
  ...
  window.trx_addons_sticky_observer_create = function (
    id,
    obj,
    callback,
    args
    ...
    if (
      ...
      if (typeof trx_addons_sticky_observers[id] == "undefined") {
        ...
        trx_addons_sticky_observers[id] = {
          observer: new IntersectionObserver(function (entries) {
            entries.forEach(function (entry) {
              ...
              jQuery(entry.target).toggleClass(
                "trx_addons_is_sticky",
                is_sticky
              );
              if (callback && typeof callback === "function") {
        ...
        trx_addons_sticky_observers[id].observer.observe(
          trx_addons_sticky_observers[id].obj
        );
      }
      return true;
    }
  ...
  window.trx_addons_add_filter = function (filter, callback, priority) {
    ...
    if (
      ...
      typeof wp.hooks.addFilter != "undefined"
    ) {
  ...
  window.trx_addons_apply_filters = function (
    filter,
    arg1,
    arg2,
    arg3,
    ...
    if (
      ...
      typeof wp != "undefined" &&
      typeof wp.hooks != "undefined" &&
      typeof wp.hooks.applyFilters != "undefined"
    ) {
      arg1 = wp.hooks.applyFilters(
        filter,
        arg1,
        arg2,
        arg3,
  ...
  window.trx_addons_add_action = function (action, callback, priority) {
    if (
      ...
      typeof wp != "undefined" &&
      typeof wp.hooks != "undefined" &&
      typeof wp.hooks.addAction != "undefined"
    ) {
      wp.hooks.addAction(
        action,
        "trx_addons",
        callback,
        priority == undefined ? 10 : priority
      );
    } else {
      trx_addons_add_filter(action, callback, priority);
    }
  };
  window.trx_addons_do_action = function (
    action,
    arg1,
    arg2,
    arg3,
    ...
    if (
      ...
      typeof wp != "undefined" &&
      typeof wp.hooks != "undefined" &&
      typeof wp.hooks.doAction != "undefined"
    ) {
      wp.hooks.doAction(
        action,
        arg1,
...
(function () {
  ...
  jQuery(document).ready(function () {
    ...
    function update_jquery_links(e) {
      ...
      $animated_elements = jQuery(
        '[data-post-animation^="animated"]:not(.animated)'
      );
      $animated_hover = jQuery(
        '[data-hover-animation^="animated"]:not(.animated)'
      );
      $video_sticky = jQuery(".trx_addons_video_sticky");
      $fixed_columns = jQuery(".sc_column_fixed");
      $stack_sections = jQuery(
        ".sc_stack_section_on:not(.elementor-element-edit-mode)"
      );
      if ($stack_sections.length > 0) {
        $body.addClass(
          "sc_stack_section_present " +
            (window.trx_addons_browser_is_ios() ? "ua_ios" : "ua_not_ios")
        );
      }
      ...
      if (trx_addons_browser_is_ios()) {
        $video_hovers_yt = jQuery(
          '.video_hover[data-video*="youtu"],.post_video_hover[data-video*="youtu"],.trx_addons_video_list_controller_item[data-video*="youtu"]'
        );
    ...
    window.trx_addons_show_preloader = function () {
      if (
        $page_preloader.length > 0 &&
        (!jQuery.browser || !jQuery.browser.safari) &&
        !$body.hasClass("ua_safari")
      ) {
        ...
        if (!$page_preloader.data("done")) {
          $page_preloader
            ...
            .animate({ opacity: $page_preloader.data("opacity") }, 300);
    ...
    window.trx_addons_hide_preloader = function () {
      if ($page_preloader.length > 0) {
        ...
        if (
          !$page_preloader.data("done") &&
          $page_preloader.css("opacity") > 0
        ) {
    ...
    function trx_addons_init_actions() {
      ...
      if (
        TRX_ADDONS_STORAGE["vc_edit_mode"] &&
      ...
      if (
        ...
        setTimeout(function () {
          ...
          if (
            ...
            TRX_ADDONS_STORAGE["animate_to_mc4wp_form_submitted"] &&
            ($mc4form = jQuery(".mc4wp-form-submitted")).length
          ) {
            ...
            if (obj.length > 0) {
              ...
              if (
                !isNaN(off) &&
                ((trx_addons_fixed_rows_height() > 0 &&
                  off - trx_addons_window_scroll_top() <
                    trx_addons_fixed_rows_height() + 60) ||
                  trx_addons_window_scroll_top() === 0)
              ) {
                trx_addons_document_animate_to(
                  off - trx_addons_fixed_rows_height() - 60
                );
              }
            }
          }
      ...
      function trx_addons_add_handlers() {
        ...
        if (
          TRX_ADDONS_STORAGE["smooth_scroll"] > 0 &&
          typeof TweenMax != "undefined"
        ) {
          var $scroll_target = $window;
          var scroll_time = trx_addons_apply_filters(
            "trx_addons_filter_smooth_scroll_time",
            0.8
          );
          var scroll_distance = trx_addons_apply_filters(
            "trx_addons_filter_smooth_scroll_distance",
            400
          );
          var ie_mobile = -1 !== navigator.userAgent.indexOf("IEMobile"),
            is_mobile =
              trx_addons_browser_is_mobile() || $body.hasClass("ua_mobile");
          var scroll_busy = false;
          var scroll_coef = trx_addons_apply_filters(
            "trx_addons_filter_smooth_scroll_coef",
            { start: 0.2, multi: 1.5, max: 1.5, value: 0.2 }
          );
          var scroll_delta = 0;
          var scroll_tween = null;
          var clear_scroll_busy = trx_addons_throttle(
            function () {
              scroll_busy = false;
              if (scroll_tween) {
                scroll_tween.kill();
          ...
          window.smooth_scroll_listener = function (event) {
            ...
            if (!scroll_busy) {
              ...
              while (depth-- >= 0) {
                if (
                  ["scroll", "auto"].indexOf($target.css("overflow-y")) >= 0
                ) {
            ...
            scroll_coef.value =
              scroll_busy && scroll_delta * delta > 0
                ? Math.min(
                    scroll_coef.max,
                    scroll_coef.value * scroll_coef.multi
                  )
                : scroll_coef.start;
            scroll_delta = delta;
            ...
            var scroll_to =
              scroll_top -
              parseInt(delta * scroll_coef.value * scroll_distance, 10);
            scroll_busy = true;
            if (scroll_tween) {
              scroll_tween.kill();
            }
            scroll_tween = TweenMax.to($scroll_target, scroll_time, {
              ...
              onInterrupt: function () {
                ...
                scroll_tween = null;
                trx_addons_do_action(
                  "trx_addons_action_smooth_scroll_interrupt",
                  this,
                  $scroll_target,
                  scroll_to,
                  scroll_time
              ...
              onComplete: function () {
                ...
                scroll_tween = null;
                trx_addons_do_action(
                  "trx_addons_action_smooth_scroll_complete",
                  this,
                  $scroll_target,
                  scroll_to,
                  scroll_time
              ...
              onUpdate: function () {
                trx_addons_do_action(
                  "trx_addons_action_smooth_scroll_update",
                  this,
                  $scroll_target,
                  scroll_to,
                  scroll_time
              ...
              scrollTo: { y: scroll_to, autoKill: true },
              ease: trx_addons_apply_filters(
                "trx_addons_filter_smooth_scroll_ease",
                Power1.easeOut
              ),
              overwrite: true,
              tween_value: 100,
        ...
        typeof window.wpb_prepare_tab_content == "function" &&
          ...
          (window.wpb_prepare_tab_content_old =
    ...
    function trx_addons_ready_actions(e, container) {
          ...
          if (trx_addons_is_local_link(href)) {
            ...
            if (pos >= 0) {
              ...
              if (jQuery(href).length > 0) {
                ...
                $document.trigger("action.trx_addons_inner_links_click", [
                  link_obj,
                  e,
                ]);
        ...
        if ($accordion.length > 0) {
          $accordion.each(function () {
            ...
            accordion.addClass("inited").accordion({
              ...
              activate: function (event, ui) {
                if (ui.oldPanel.length > 0 && !ready_busy) {
                  $document.trigger("action.deactivate_accordion", [
                    ui.oldPanel,
                  ]);
                }
                if (ui.newPanel.length > 0 && !ready_busy) {
      ...
      if (jQuery.ui && jQuery.ui.slider) {
        var $range_slider = container.find(
          ".trx_addons_range_slider:not(.inited)"
        );
          ...
          $range_slider.each(function () {
            ...
            var init_obj = {
              ...
              slide: function (event, ui) {
                trx_addons_range_slider_update_current_values_position(
                  ui,
            ...
            function trx_addons_range_slider_update_current_values_position(
              ui,
              cur_values
            ) {
      ...
      var $play_on_hover = container.find(
        ".trx_addons_video_hover:not(.inited)"
      );
      ...
      if ($play_on_hover.length > 0) {
        $play_on_hover
          ...
          .on("mouseenter touchstart", function (e) {
            var $self = jQuery(this);
            if (
              !$self.hasClass("trx_addons_video_hover_play") &&
              (e.type != "touchstart" || !touchstart_just_fired)
            ) {
              $self
          ...
          .on("mouseleave touchstart", function (e) {
            ...
            if (
              ...
              $self
                ...
                .one(
                  typeof window.trx_addons_transition_end != "undefined"
                    ? trx_addons_transition_end()
                    : "transitionend",
                  function () {
        ...
        var $play_on_hover_autoplay = $play_on_hover.find(
          'video[data-autoplay="1"]'
        );
        if ($play_on_hover_autoplay.length > 0) {
          trx_addons_intersection_observer_add(
            $play_on_hover_autoplay,
            function (item, enter) {
      ...
      var $video_hover = container.find(
        ...
        $video_hover
          ...
          .on("click", function (e) {
            ...
            if (slider.length > 0) {
              ...
              if (
                typeof TRX_ADDONS_STORAGE["swipers"][id].autoplay != "undefined"
              ) {
      ...
      var $video_controller = container.find(
        ".trx_addons_video_list_controller_wrap:not(.inited)"
      );
      if ($video_controller.length > 0) {
        $video_controller
          ...
          .on(
            "click",
            '.trx_addons_video_list_controller_item > a[href="#"]',
            function (e) {
              e.preventDefault();
              ...
              if (
                ...
                item
                  .parent()
                  .find(".trx_addons_video_list_controller_item_active")
                  .removeClass("trx_addons_video_list_controller_item_active");
                item.addClass("trx_addons_video_list_controller_item_active");
                var autoplay = video_wrap.find(".with_video_autoplay");
                if (autoplay.length) {
                  autoplay
                    .removeClass(
                      "with_video_autoplay video_autoplay_inited video_autoplay_started"
                    )
                    .find("video_frame_controls")
                    .remove();
                ...
                video_wrap
                  ...
                  .fadeTo(300, 1, function () {
                    if (
                      item.data("autoplay") > 0 &&
                      video_wrap.find(".trx_addons_video_player .video_hover")
                        .length > 0
                    ) {
                      video_wrap
                        .find(".trx_addons_video_player .video_hover")
                        .eq(0)
                        .trigger("click");
                    }
                  });
        ...
        setTimeout(function () {
          ...
          "trx_addons_filter_video_controller_preload_images_timeout",
          0
        ));
      }
        ...
        video
          ...
          .on("click", function (e) {
            ...
            if (_video_sticky_fade) {
              video
                .addClass("trx_addons_video_sticky_on_fade")
                .stop()
                .animate({ opacity: 0 }, 300, function () {
                  video
                    .parents(".post_featured")
                    .removeClass("with_video_sticky_on");
                  video
                    .height("auto")
                    .removeClass(
                      "trx_addons_video_sticky trx_addons_video_sticky_on trx_addons_video_sticky_on_fade"
                    )
                    .stop()
              ...
              video
                .height("auto")
                .removeClass(
                  "trx_addons_video_sticky trx_addons_video_sticky_on"
                );
              $video_sticky = jQuery(".trx_addons_video_sticky");
              $window.trigger("resize");
            }
            ...
            if (!enter) {
              if (
                ...
                if (video.find(".video_frame").html().trim().length > 30) {
                  video
                    .parents(".post_featured")
                    .addClass("with_video_sticky_on");
                  if (_video_sticky_fade) {
                    video
                      .height(video_height)
                      .css("opacity", 0)
                      .addClass("trx_addons_video_sticky_on")
                      .stop()
                      .animate({ opacity: 1 }, 500);
                  } else {
                    video
              ...
              if (
                video.hasClass("trx_addons_video_sticky_on") &&
                !video.hasClass("trx_addons_video_sticky_on_fade")
              ) {
                if (_video_sticky_fade) {
                  video
                    .addClass("trx_addons_video_sticky_on_fade")
                    ...
                    .animate({ opacity: 0 }, 300, function () {
                      ...
                      video
                        .height("auto")
                        .removeClass(
                          "trx_addons_video_sticky_on trx_addons_video_sticky_on_fade"
                        )
                        .stop()
                        .animate({ opacity: 1 }, 500);
                      $window.trigger("resize");
                  ...
                  video
                    .height("auto")
                    .removeClass("trx_addons_video_sticky_on");
                  $window.trigger("resize");
                }
              }
            }
      ...
      if (TRX_ADDONS_STORAGE["popup_engine"] == "pretty") {
        container
          ...
          .each(function () {
            var $self = jQuery(this);
            if (
              !$self.parent().hasClass("woocommerce-product-gallery__image")
            ) {
        ...
        if (images.length > 0) {
          try {
            images.prettyPhoto(
              trx_addons_apply_filters("pretty-init-images-params", {
                social_tools: "",
                theme: "facebook",
                deeplinking: false,
              })
        ...
        TRX_ADDONS_STORAGE["popup_engine"] == "magnific" &&
        ...
        container
          ...
          .each(function () {
            ...
            if (
              trx_addons_apply_filters(
                "mfp-init-images-allow",
                $self.closest(".cq-dagallery").length === 0 &&
                  $self.closest(".woocommerce-product-gallery__image")
                    .length === 0 &&
        ...
        if (images.length > 0) {
          ...
          try {
            images.magnificPopup(
              trx_addons_apply_filters("mfp-init-images-params", {
                type: "image",
                ...
                closeMarkup:
                  '<button title="%title%" aria-label="%title%" type="button" class="mfp-close"><span class="mfp-close-icon">&#215;</span></button>',
                ...
                image: {
                  ...
                  titleSrc: function (item) {
                    var title = "",
                      $el =
                        typeof item.el != "undefined" ? jQuery(item.el) : null;
                    if ($el && $el.length > 0) {
                      var $next = $el.next();
                      if (
                        $next.length > 0 &&
                        $next.get(0).tagName == "FIGCAPTION"
                      ) {
                        title = $next.text();
                  ...
                  opener: function (openerElement) {
                    if (!openerElement.is("img")) {
                      if (
                        ...
                        openerElement.parent().parent().find("img").length > 0
                      )
                        openerElement = openerElement
                          .parent()
                          .parent()
        ...
        container
          .find(".sc_layouts_popup:not(.inited),.sc_layouts_panel:not(.inited)")
          .each(function () {
            var $self = jQuery(this),
              id = $self.attr("id"),
              show = false;
            ...
            var is_panel = $self.hasClass("sc_layouts_panel"),
              link = jQuery(
                'a[href="#' +
                  ...
                  '"],' +
                  (is_panel
                    ? '.trx_addons_panel_link[data-panel-id="' + id + '"]'
                    : '.trx_addons_popup_link[data-popup-id="' + id + '"]')
              );
            if (link.length === 0) {
              $body.append(
                '<a href="#' + id + '" class="trx_addons_hidden"></a>'
              );
            ...
            if ($self.hasClass("sc_layouts_show_on_page_load")) {
              ...
              $self.hasClass("sc_layouts_show_on_page_load_once") &&
              trx_addons_get_cookie(
                "trx_addons_show_on_page_load_once_" + id
              ) != "1"
            ) {
              trx_addons_set_cookie(
                "trx_addons_show_on_page_load_once_" + id,
              ...
              $self.hasClass("sc_layouts_show_on_page_close") &&
              trx_addons_get_cookie("trx_addons_show_on_page_close_" + id) !=
                "1"
            ) {
            ...
            link
              .addClass(
                is_panel ? "trx_addons_panel_link" : "trx_addons_popup_link"
              )
              .data("panel", $self);
            $self
              .addClass("inited")
          ...
          $window.on("mousemove", function (e) {
            ...
            if (y < trx_addons_adminbar_height() + 15) {
              ...
              on_leaving_site.forEach(function (item) {
                ...
                trx_addons_set_cookie(
                  "trx_addons_show_on_page_close_" + item.id,
                  "1"
                );
              });
            }
        ...
        container
          .find(
            trx_addons_apply_filters(
              "mfp-init-popup-selector",
              ".trx_addons_popup_link:not(.popup_inited)"
            )
          )
          .addClass("popup_inited")
          .magnificPopup(
            trx_addons_apply_filters("mfp-init-popup-params", {
              type: "inline",
              focus: "input",
              removalDelay: trx_addons_apply_filters(
                "trx_addons_filter_close_popup_timeout",
                0
              ),
              tLoading: TRX_ADDONS_STORAGE["msg_magnific_loading"],
              tClose: TRX_ADDONS_STORAGE["msg_magnific_close"],
              ...
              callbacks: {
                beforeAppend: function () {
                  var $mfp = this;
                  $document.trigger("action.prepare_popup_elements", [
                    $mfp.content,
                    $mfp,
                  ]);
                },
                beforeOpen: function () {
                  var $mfp = this;
                  $document.trigger("action.open_popup_elements", [
                    $mfp.content,
                  ]);
                  var wrap_animation_in = mfp_get_animation(
                      $mfp,
                      "wrap",
                      "open"
                    ),
                ...
                beforeClose: function () {
                  var $mfp = this;
                  $document.trigger("action.close_popup_elements", [
                    $mfp.content,
                  ]);
                  var wrap_animation_in = mfp_get_animation(
                      $mfp,
                    ...
                    bg_animation_in = mfp_get_animation($mfp, "bg", "open"),
                    wrap_animation_out = mfp_get_animation(
                      $mfp,
                      "wrap",
                      "close"
                    ),
                    ...
                    delay = wrap_animation_out
                      ? trx_addons_apply_filters(
                          "mfp-init-popup-animations-duration",
                  ...
                  if (bg_animation_out) {
                    setTimeout(function () {
                      $mfp.bgOverlay
                        .removeClass(bg_animation_in)
                        .addClass(bg_animation_out);
                    }, $mfp.st.removalDelay - delay);
                  }
                  if (wrap_animation_out) {
                    setTimeout(function () {
                      $mfp.wrap
                        .removeClass(wrap_animation_in)
                        .addClass(wrap_animation_out);
                    }, $mfp.st.removalDelay - delay);
                  }
                },
                close: function () {
        ...
        var mfp_get_animation = function (mfp, item, event, defa) {
          ...
          return mfp.st.el.attr(
            "data-popup-" + item + "-" + event + "-animation"
          )
            ? mfp.st.el.attr("data-popup-" + item + "-" + event + "-animation")
            : trx_addons_apply_filters(
                "mfp-init-popup-" + item + "-" + event + "-animation",
                defa ? defa : defaults[item + "_" + event]
              );
        };
        container
          .find(
            trx_addons_apply_filters(
              "init-panel-selector",
              ".trx_addons_panel_link:not(.panel_inited)"
            )
          )
          ...
          .on("click", function (e) {
            ...
            if (!panel.hasClass("sc_layouts_panel_opened")) {
              ...
              if (panel.prev().hasClass("sc_layouts_panel_hide_content"))
                panel.prev().addClass("sc_layouts_panel_opened");
              $body.addClass(
                "sc_layouts_panel_opened sc_layouts_panel_opened_" +
        ...
        container
          .find(".sc_layouts_panel_hide_content:not(.inited)")
          .addClass("inited")
          .on("click", function (e) {
        ...
        window.trx_addons_close_panel = function (panel) {
          ...
          setTimeout(function () {
            ...
            $body.removeClass(
              "sc_layouts_panel_opened sc_layouts_panel_opened_left sc_layouts_panel_opened_right sc_layouts_panel_opened_top sc_layouts_panel_opened_bottom"
            );
            var panel_class = panel.data("panel-class");
            if (panel_class) {
            ...
            "trx_addons_filter_close_panel_timeout",
            panel.hasClass("sc_layouts_panel") &&
              panel.data("animation-delay") !== undefined
              ? panel.data("animation-delay")
              : 0,
            panel
          ));
        };
        window.trx_addons_prepare_popup_content = function (
          container,
          autoplay
        ) {
          ...
          if (wrapper.data("popup-content") === undefined) {
            ...
            if (
              ...
              (iframe.length &&
                ((iframe.data("src") &&
                  iframe.data("src").search(/(youtu|vimeo|daily|facebook)/i) >
                    0) ||
                  (iframe.attr("src") &&
                    iframe.attr("src").search(/(youtu|vimeo|daily|facebook)/i) >
                      0)))
            ) {
          ...
          setTimeout(function () {
            jQuery
              ...
              .done(function (response) {
                ...
                if (rez.error === "") {
                  jQuery(
                    ".post_meta_single .post_meta_views .post_meta_number,.sc_layouts_title_meta .post_meta_views .post_meta_number"
                  ).html(rez.counter);
                }
      ...
      var $likes = container.find(
        "a.post_meta_likes:not(.inited),a.comment_counters_likes:not(.inited)"
      );
      if ($likes.length > 0) {
        ...
        $likes.addClass("inited").on("click", function (e) {
          if (!likes_busy) {
            ...
            var post_id = button.hasClass("post_meta_likes")
              ? button.data("postid")
              : button.data("commentid");
            var cookie_likes = trx_addons_get_cookie(
              button.hasClass("post_meta_likes")
                ? "trx_addons_likes"
            ...
            jQuery
              .post(TRX_ADDONS_STORAGE["ajax_url"], {
                action: button.hasClass("post_meta_likes")
                  ? "post_counter"
                  : "comment_counter",
                nonce: TRX_ADDONS_STORAGE["ajax_nonce"],
                post_id: post_id,
      ...
      var $share_caption = container.find(
        ".socials_share .socials_caption:not(.inited)"
      );
      ...
      var $share_items = container.find(
        ".socials_share .social_items:not(.inited)"
      );
      ...
      jQuery(".sc_layouts_menu_nav:not(.inited_cache)").each(function () {
        ...
        if (
          $self.find(".current-menu-item").length == 0 ||
          $body.hasClass("blog_template")
        ) {
      ...
      if (
        ...
        var href2 =
          href.indexOf("#") == -1 && href.indexOf("?") == -1 ? href + "/" : "";
      ...
      function trx_addons_comments_validate(form) {
        ...
        if (
          form.find('.comments_author input[aria-required="true"]').length > 0
        ) {
        ...
        if (
          form.find('.comments_email input[aria-required="true"]').length > 0
        ) {
          comments_args.rules.push({
            field: "email",
            min_length: {
      ...
      if (
        TRX_ADDONS_STORAGE["disable_animation_on_mobile"] &&
        $body.hasClass("ua_mobile")
      ) {
        jQuery('[data-post-animation^="animated"]').removeAttr(
          "data-post-animation"
        );
        jQuery('[data-hover-animation^="animated"]').removeAttr(
        ...
        $parallax_wrap.each(function () {
          ...
          if (speed !== 0 && !$self.hasClass("parallax_inited")) {
            ...
            var selector =
              "#" +
              id +
              ($self.find("> .wp-caption").length > 0 ? ">.wp-caption" : "") +
              ">img";
            ...
            var prc =
              trx_addons_document_height() > trx_addons_window_height()
                ? Math.min(
                    ...
                    Math.max(
                      0,
                      (trx_addons_window_scroll_top() /
                        (trx_addons_document_height() -
    ...
    function trx_addons_intersection_observer_init() {
      ...
      if (typeof IntersectionObserver != "undefined") {
        if (typeof TRX_ADDONS_STORAGE["intersection_observer"] == "undefined") {
          TRX_ADDONS_STORAGE["intersection_observer"] =
            new IntersectionObserver(
              function (entries) {
                entries.forEach(function (entry) {
                  trx_addons_intersection_observer_in_out(
                    jQuery(entry.target),
        ...
        $window.on("scroll", function () {
          if (
            typeof TRX_ADDONS_STORAGE["intersection_observer_items"] !=
            "undefined"
          ) {
            for (var i in TRX_ADDONS_STORAGE["intersection_observer_items"]) {
              if (
                !TRX_ADDONS_STORAGE["intersection_observer_items"][i] ||
                TRX_ADDONS_STORAGE["intersection_observer_items"][i].length ===
                  0
              ) {
                continue;
              }
              ...
              trx_addons_intersection_observer_in_out(
                item,
                item_top + item_height > trx_addons_window_scroll_top() &&
                  item_top <
                    trx_addons_window_scroll_top() + trx_addons_window_height()
                  ? "in"
                  : "out"
              );
            }
      ...
      window.trx_addons_intersection_observer_in_out = function (
        item,
        state,
        entry
      ) {
      ...
      window.trx_addons_intersection_observer_add = function (items, callback) {
        items.each(function () {
          ...
          if (!$self.hasClass("trx_addons_intersection_inited")) {
            ...
            if (
              typeof TRX_ADDONS_STORAGE["intersection_observer"] !== "undefined"
            ) {
        ...
        items.each(function () {
          ...
          if ($self.hasClass("trx_addons_intersection_inited")) {
            ...
            if (
              ...
              TRX_ADDONS_STORAGE["intersection_observer"].unobserve(
                $self.get(0)
              );
            }
          }
    ...
    window.trx_addons_scroll_actions = function () {
      ...
      if (
        ["top", "bottom", "fixed"].indexOf(
          TRX_ADDONS_STORAGE["scroll_progress"]
        ) >= 0
      ) {
      ...
      $show_on_scroll.each(function () {
        ...
        if (item.hasClass("trx_addons_in_viewport")) {
          if (
            item.offset().top <
            trx_addons_window_scroll_top() + trx_addons_window_height() * 0.75
          ) {
            item
              .removeClass("trx_addons_show_on_scroll")
    ...
    function trx_addons_show_scroll_progress() {
      if (TRX_ADDONS_STORAGE["scroll_progress_status"] == undefined) {
        $body.append(
          '<div class="scroll_progress_wrap scroll_progress_' +
            TRX_ADDONS_STORAGE["scroll_progress"] +
            '"><span class="scroll_progress_status"></span></div>'
        );
        TRX_ADDONS_STORAGE["scroll_progress_status"] = jQuery(
        ...
        TRX_ADDONS_STORAGE["scroll_progress_status"].on("click", function (e) {
          ...
          if (
            TRX_ADDONS_STORAGE["scroll_posts"] != undefined &&
            TRX_ADDONS_STORAGE["scroll_posts"].length > 0
          ) {
      ...
      if (
        ...
        new_width =
          (st < 10
            ? 0
            : Math.min(100, Math.round(((st + wh / 2 - pt) * 100) / ph))) + "%";
        TRX_ADDONS_STORAGE["scroll_progress_status"].width(new_width);
      } else {
        new_width =
          Math.min(
            100,
            Math.round((st * 100) / (trx_addons_document_height() - wh))
          ) + "%";
        TRX_ADDONS_STORAGE["scroll_progress_status"].width(new_width);
      }
    }
    function trx_addons_detect_current_scroll_post() {
      ...
      if (!cur_post) {
        cur_post = TRX_ADDONS_STORAGE["scroll_posts"].eq(
          TRX_ADDONS_STORAGE["scroll_posts"].length - 1
        );
      }
      return cur_post;
    }
    ...
    function trx_addons_get_scroll_posts() {
      TRX_ADDONS_STORAGE["scroll_posts"] =
        (TRX_ADDONS_STORAGE["scroll_posts"] !== undefined &&
          TRX_ADDONS_STORAGE["scroll_posts"].length > 0) ||
      ...
      var trx_addons_fix_column = function (e, cont) {
        ...
        $fixed_columns.each(function () {
          ...
          if (
            ...
            trx_addons_window_width() <
            TRX_ADDONS_STORAGE["mobile_breakpoint_fixedcolumns_off"]
          ) {
            ...
            if (
              ...
              trx_addons_window_scroll_top() + trx_addons_fixed_rows_height() >
                row_top
            ) {
              var col_init = {
                position: "undefined",
              ...
              if (
                typeof TRX_ADDONS_STORAGE["scroll_offset_last"] == "undefined"
              ) {
                TRX_ADDONS_STORAGE["col_top_last"] = row_top;
                TRX_ADDONS_STORAGE["scroll_offset_last"] =
                  trx_addons_window_scroll_top();
              ...
              var col_big =
                  col_height + 30 >=
                  trx_addons_window_height() - trx_addons_fixed_rows_height(),
              ...
              if (col_init.position != "undefined") {
                ...
                if (!col.prev().hasClass("sc_column_fixed_placeholder")) {
                  ...
                  col.before(
                    '<div class="sc_column_fixed_placeholder ' +
                      col.attr("class").replace("sc_column_fixed", "") +
                      '"' +
                      (col.data("col")
                ...
                col_init.width =
                  col.prev().width() +
                  parseFloat(col.prev().css("paddingLeft")) +
                  parseFloat(col.prev().css("paddingRight"));
                if (
                  force ||
    ...
    function trx_addons_stack_section(e, cont) {
      ...
      $stack_sections.each(function (idx) {
        var row = jQuery(this),
          ...
          row_top = 0,
          use_sticky =
            $body.hasClass("fixed_blocks_sticky") &&
            row.hasClass("sc_stack_section_effect_slide");
        if (
          trx_addons_window_width() <
          TRX_ADDONS_STORAGE["mobile_breakpoint_stacksections_off"]
        ) {
          ...
          if (row.hasClass("sc_stack_section_fixed")) {
            row_holder = row.prev();
            row_height = use_sticky
              ? row.outerHeight()
              : row_holder.outerHeight();
              ...
              if (row.hasClass("sc_stack_section_effect_fade")) {
                if (wso - row_top <= row_height) {
                  row.css({
                    opacity: Math.max(
                      0,
                      Math.min(1, (wso - row_top) / row_height)
                    ),
                  });
                } else {
                  row.css({ opacity: 1 });
            ...
            if (row_top <= wso) {
              ...
              row
                .before(
                  '<div class="sc_stack_section_placeholder"' +
                    (!use_sticky
    ...
    function trx_addons_stack_section_zoom() {
      $stack_sections.each(function () {
        var targetElement = jQuery(this),
          ...
          transparency = targetElement.hasClass(
            "sc_stack_section_transparency_on"
          );
        ...
        if (
          !triggerElement.length ||
          (triggerElement.hasClass("sc_stack_section_on") &&
            !triggerElement.hasClass("sc_stack_section_effect_slide"))
        ) {
          return;
        }
        ...
        var timeline = triggerElement.data("stack-section-timeline")
          ? triggerElement.data("stack-section-timeline")
          : null;
        if (timeline) {
          timeline.kill();
        }
    ...
    function trx_addons_resize_video(e, cont) {
      if ($video_tags.length > 0) {
        $video_tags.each(function () {
          ...
          if (
            (!TRX_ADDONS_STORAGE["resize_tag_video"] &&
        ...
        $iframe_tags.each(function () {
          ...
          var iframe = $self.eq(0),
            iframe_src = iframe.attr("src")
              ? iframe.attr("src")
              : iframe.data("src");
          if (iframe_src === undefined || iframe_src.indexOf("soundcloud") > 0)
            return;
    ...
    jQuery.fn.isInViewport = function () {
      ...
      var rez = trx_addons_apply_filters(
        "trx_addons_filter_element_in_viewport",
        $panel.length === 0 || $panel.hasClass("sc_panel_thumb_active"),
        $self
      );
      if (rez) {
      ...
      if (typeof YTdeferred != "undefined") {
        YTdeferred.done(function (YT) {
          $video_autoplay_yt.each(function () {
            ...
            if (
              $self.parents(
                ".sc_layouts_submenu:not(.layouts_inited):not(:visible)"
              ).length
            )
              return;
            ...
            if (!isInit) {
              ...
              if (!videoID) {
                var src = (
                  $self.data("src") ? $self.data("src") : $self.attr("src")
                ).split("?");
              ...
              $frame.append(
                '<span class="video_frame_overlay"></span>' +
                  ...
                  '<a class="video_frame_control_stop video_frame_link" href="https://youtube.com/watch?v=' +
                  videoID +
                  '" target="_blank"></a>' +
                  ...
                  ($self.attr("allow") &&
                  $self.attr("allow").indexOf("autoplay") >= 0
                    ? "pause"
                    : "play") +
                  '"></span>' +
                  "</span>"
              ...
              $frame
                .find(".video_frame_control_state")
                .on("click", function () {
                  var $self = jQuery(this);
                  $self.toggleClass(
                    "video_frame_control_state_play video_frame_control_state_pause"
                  );
                  if ($self.hasClass("video_frame_control_state_play")) {
                    ...
                    if (typeof players[id].pauseVideo == "function")
              ...
              $frame.find(".video_frame_control_stop").on("click", function () {
                var $self = jQuery(this);
                $self
                  .siblings(".video_frame_control_state")
                  .removeClass("video_frame_control_state_play")
                  .addClass("video_frame_control_state_pause");
                $self.addClass("video_frame_control_state_upause");
                if (typeof players[id].pauseVideo == "function")
                  players[id].pauseVideo();
              });
              $frame
                .find(".video_frame_control_volume")
                .on("click", function () {
                  var $self = jQuery(this);
                  $self.toggleClass(
                    "video_frame_control_volume_mute video_frame_control_volume_unmute"
                  );
                  if ($self.hasClass("video_frame_control_volume_unmute")) {
                    if (typeof players[id].unMute == "function")
                      players[id].unMute();
                  } else {
                    if (typeof players[id].mute == "function")
                      players[id].mute();
                  }
                });
              attrs[id] = {
                videoId: videoID,
                startSeconds:
                  $self.data("video-start") ||
                  trx_addons_apply_filters(
                    "trx_addons_filter_youtube_autoplay_start_seconds",
                    -1
                  ),
                suggestedQuality: "hd720",
              };
            ...
            if (
              isInit &&
              isInView &&
              !$frame.hasClass(".video_frame_visible")
            ) {
              setTimeout(function () {
                ...
                "trx_addons_filter_video_frame_timeout",
                0
              ));
            }
            ...
            if (
              isInit &&
              typeof players[id].playVideo == "function" &&
              !control.hasClass("video_frame_control_state_upause") &&
              $sticky.length === 0
            ) {
              if (
                isInView &&
                control.hasClass("video_frame_control_state_pause")
              ) {
                control
                  .removeClass("video_frame_control_state_pause")
                  .addClass("video_frame_control_state_play");
                players[id].playVideo();
              }
              if (
                !isInView &&
                control.hasClass("video_frame_control_state_play")
              ) {
                control
                  .removeClass("video_frame_control_state_play")
                  .addClass("video_frame_control_state_pause");
    ...
    if (!initEvents) {
      initEvents = true;
      $document.on(
        "action.init_hidden_elements action.got_ajax_response action.after_show_submenu action.after_hide_submenu",
        trx_addons_debounce(function (e) {
          initYoutubePlayer();
        }, 50)
      );
            ...
            if (!TRX_ADDONS_STORAGE["double_opt_in_registration"]) {
              setTimeout(function () {
                if (
                  rez.redirect_to !== "" &&
                  trx_addons_apply_filters(
                    "trx_addons_filter_redirect_after_user_registration",
                    false,
                    rez.redirect_to
                  )
                ) {
...
jQuery(document).ready(function () {
  ...
  $document.on("action.init_hidden_elements", function () {
    ...
    jQuery(".sc_item_filters:not(.inited),.sc_item_pagination:not(.inited)")
      ...
      .on("click", "a", function (e) {
        ...
        if (
          ...
          !TRX_ADDONS_STORAGE["pagination_busy"]
        ) {
          var link_wrap = link.parents(".sc_item_filters,.sc_item_pagination"),
            load_more = link_wrap.hasClass("sc_item_pagination_load_more"),
            sc = link_wrap.parent(),
            set_min_height = trx_addons_apply_filters(
              "trx_addons_filter_set_min_height_on_switch_tabs",
              true,
              sc
            ),
          ...
          if (link_wrap.hasClass("sc_item_filters")) {
            ...
            link_active.data(
              "pagination",
              pagination_wrap.length > 0 ? pagination_wrap.clone() : false
            );
          }
          if (
            link_wrap.hasClass("sc_item_filters") &&
            link.data("posts") !== undefined
          ) {
            if (set_min_height) {
              sc.data("min-height", sc.css("min-height")).css(
                "min-height",
                sc.height()
              );
            }
            link_active
              .removeClass("active")
              .parent()
              .removeClass("sc_item_filters_tabs_active");
            link
              .addClass("active")
              .parent()
              .addClass("sc_item_filters_tabs_active");
            ...
            posts.animate({ opacity: 0 }, 200, function () {
              ...
              posts
                .find(".swiper-container-initialized")
            ...
            jQuery
              .post(TRX_ADDONS_STORAGE["ajax_url"], {
                ...
                nonce: TRX_ADDONS_STORAGE["ajax_nonce"],
                params:
                  pagination_wrap.length > 0
                    ? pagination_wrap.data("params")
                    : link_wrap.data("params"),
                page:
                  link.parents(".sc_item_filters").length > 0
                    ? 1
                    : link.data("page"),
                filters_active:
                  link.parents(".sc_item_filters").length > 0
                    ? link.data("tab")
                    : link_wrap.siblings(".sc_item_filters").find("a.active")
                        .length > 0
                    ? link_wrap
                        .siblings(".sc_item_filters")
                        .find("a.active")
                        .data("tab")
                    : "all",
              })
                ...
                if (rez.error === "") {
                  ...
                  if (load_more) {
                    ...
                    if (posts.find('[class*="_tabs_list_item"]').length > 0) {
                      posts
                        .find('[class*="_tabs_list_item"]')
                        .parent()
                        .append(
                          jQuery(rez.data)
                            .find(
                              '.sc_item_posts_container [class*="_tabs_list_item"]'
                            )
                            .parent()
                            .html()
                        );
                      posts
                        .find('[class$="_tabs_content"]')
                        .append(
                          jQuery(rez.data)
                            .find(
                              '.sc_item_posts_container [class$="_tabs_content"]'
                            )
                            .html()
                        );
                      posts
                        .find('[class*="_tabs_list_item_active"]')
                        .each(function (idx) {
                          if (idx > 0) {
                            ...
                            for (var i = 0; i < classes.length; i++) {
                              if (
                                classes[i].indexOf("_tabs_list_item_active") > 0
                              ) {
                                classes[i] = "";
                                found = true;
                                break;
                            ...
                            if (found)
                              jQuery(this).attr("class", classes.join(" "));
                          }
                        });
                      posts
                        .find(
                          '[class$="_tabs_content"] [class*="_item_active"]'
                        )
                        .each(function (idx) {
                          if (idx > 0) {
                            var classes = jQuery(this).attr("class").split(" "),
                      ...
                      if (posts.hasClass("masonry_wrap")) {
                        var items = jQuery(rez.data).find(
                          ".sc_item_posts_container .masonry_item"
                        );
                        if (items.length > 0) {
                          items.addClass("just_loaded_items hidden");
                          posts.append(items);
                          var just_loaded_items =
                            posts.find(".just_loaded_items");
                          ...
                          setTimeout(function () {
                            just_loaded_items.removeClass(
                              "just_loaded_items hidden"
                            );
                          }, 1000);
                        }
                      } else {
                    ...
                    if (pagination_wrap.length > 0) {
                      var new_params = jQuery(rez.data)
                        .find(".sc_item_pagination")
                        .data("params");
                      if (new_params) {
                    ...
                    posts.animate({ opacity: 0 }, 200, function () {
                      var items = jQuery(rez.data).find(
                        ".sc_item_posts_container,.sc_item_slider"
                      );
                      if (
                        items.find('[class*="trx_addons_column-"]').length > 0
                      ) {
                      ...
                      trx_addons_replace_pagination(
                        pagination_wrap,
                        jQuery(rez.data).find(".sc_item_pagination"),
                        posts
                      );
                      $document.trigger("action.after_add_content", [sc]);
                      $document.trigger("action.init_hidden_elements", [sc]);
    ...
    function trx_addons_replace_pagination(
      pagination_wrap,
      pagination_new,
      posts
    ) {
    ...
    jQuery(
      ".sc_blogger_panel .sc_blogger_item:not(.switch_panel_inited),.sc_services_panel .sc_services_item:not(.switch_panel_inited)"
    )
      ...
      .on("mouseenter", function () {
        var $self = jQuery(this),
          ...
          $old_panel = $posts
            .find(".sc_panel_thumb_active")
            .removeClass("sc_panel_thumb_active"),
          $new_panel = $posts
            .find('.sc_panel_thumb[data-thumb-number="' + num + '"]')
            .addClass("sc_panel_thumb_active");
    ...
    jQuery(".sc_cover:not(.inited)").each(function () {
      ...
      var $wrap = $self.parent().hasClass("elementor-widget-container")
          ? $self.parents(".elementor-widget").eq(0)
          : $self,
        wrap_z =
          $wrap.hasClass("elementor-widget") && $wrap.css("z-index") > 0
            ? $wrap.css("z-index")
            : "",
      ...
      if (place == "p1") {
        ...
        $placeholder = $wrap
          .parents(
            trx_addons_apply_filters(
              "trx_addons_filter_section_selectors",
              ".wp-block-columns,.elementor-section"
            )
          )
          .eq(0);
        ...
        $placeholder = $wrap
          .parents(
            trx_addons_apply_filters(
              "trx_addons_filter_column_selectors",
              ".wp-block-column,.elementor-column"
            )
          )
          .eq(0);
      }
  ...
  $document.on("action.scroll_trx_addons", function (e) {
    if (
      TRX_ADDONS_STORAGE["pagebuilder_preview_mode"] ||
      $pagination_infinite.length === 0
    ) {
      return;
    }
    var done = false;
    $pagination_infinite.each(function (idx) {
      ...
      if (
        posts.data("offset-top") + posts.data("height") <
        trx_addons_window_scroll_top() + trx_addons_window_height() * 1.5
      ) {
      ...
      if (wt <= ot + oh && wt + wh >= ot + oh) {
        obj
          ...
          .typed({
            ...
            cursorChar:
              obj.data("cursor-char") != undefined
                ? obj.data("cursor-char")
                : "|",
            typeSpeed:
              obj.data("speed") > 0
                ? (11 - Math.max(1, Math.min(10, obj.data("speed")))) * 10
                : 50,
            backDelay:
              obj.data("delay") > 0
                ? Math.max(0, Math.min(10, obj.data("delay"))) * 1000
                : 1000,
          });
      }
    });
...
(function () {
  ...
  update_jquery_links();
  if (
    typeof TRX_ADDONS_STORAGE != "undefined" &&
    TRX_ADDONS_STORAGE["popup_engine"] != "none"
  ) {
    $document.on("action.init_hidden_elements", function (e, cont) {
      cont.find("a.esgbox").attr("data-elementor-open-lightbox", "no");
      if (
        trx_addons_apply_filters(
          "trx_addons_filter_disable_elementor_lightbox",
          true
        )
      ) {
        cont
          .find(
            trx_addons_apply_filters(
              "trx_addons_filter_disable_elementor_lightbox_selector",
    ...
    if (
      ...
      if (elementorFrontend.isEditMode()) {
        var init_hidden_elements_immediately = false,
          init_hidden_elements_immediately_start = trx_addons_throttle(
            function () {
          ...
          init_hidden_elements = function ($cont) {
            if (
              $body.hasClass("cpt_layouts-template") ||
              $body.hasClass("cpt_layouts-template-default") ||
              $body.hasClass("single-cpt_layouts") ||
              $body.hasClass("single-elementor_library")
            ) {
              $body
                .find(".elementor-element.elementor-widget")
                .addClass("sc_layouts_item");
            }
            jQuery("#toc_menu").remove();
            $document.trigger("action.init_hidden_elements", [$cont]);
            if (
              $cont.parents(".elementor-section-stretched").length > 0 &&
              !trx_addons_once_resize
            ) {
              trx_addons_once_resize = true;
              $document.trigger("action.resize_trx_addons", [
                $cont.parents(".elementor-section-stretched"),
              ]);
            } else {
              $document.trigger("action.resize_trx_addons", [$cont]);
            }
        ...
        if (
          ...
          jQuery(".elementor-element.elementor-widget").addClass(
            "sc_layouts_item"
          );
        }
        ...
        jQuery(".trx_addons_layout_editor_selector_trigger:not(.inited)")
          ...
          .on("click", function (e) {
            ...
            jQuery(this)
              .parent()
              .toggleClass("trx_addons_layout_editor_selector_opened");
            e.preventDefault();
            return false;
          });
        jQuery(".trx_addons_layout_editor_selector_list_item:not(.inited)")
          ...
          .on("click", function (e) {
            ...
            $self
              .parent()
              .prevAll(".trx_addons_layout_editor_selector_trigger")
              .eq(0)
              .trigger("click");
            var link = $self
                .parent()
                .prevAll(".trx_addons_layout_editor_link")
                .eq(0),
              text = link.text();
            link
            ...
            if (layout_id && layout_type && post_id && elementor) {
              var settings = elementor.settings,
                ...
                override_name =
                  theme_slug + "_options_override_" + layout_type + "_type",
                field_name =
                  theme_slug + "_options_field_" + layout_type + "_type";
              ...
              override_name =
                theme_slug + "_options_override_" + layout_type + "_style";
              field_name =
                theme_slug + "_options_field_" + layout_type + "_style";
              if (model.attributes.hasOwnProperty(override_name)) {
                model.set(override_name, "1");
              }
              if (model.attributes.hasOwnProperty(field_name)) {
              ...
              if (
                elementor.panel.$el.find(
                  'input[data-setting="' + override_name + '"]'
                ).length === 0
              ) {
                elementor.panel.$el
                  .find("#elementor-panel-footer-settings")
                  .trigger("click");
                setTimeout(function () {
                  elementor.panel.$el
                    .find(".elementor-tab-control-advanced a")
                    .trigger("click");
                  setTimeout(function () {
                    var sec_number =
                      layout_type == "sidebar"
                    ...
                    if (sec_number > 1) {
                      elementor.panel.$el
                        .find(
                          ".elementor-control-section_theme_options_" +
                            sec_number
                        )
                        .trigger("click");
                    }
                    setTimeout(function () {
                      elementor.panel.$el
                        .find('input[data-setting="' + override_name + '"]')
                        .trigger("change");
                    }, 10);
                  }, 10);
                }, 10);
              } else {
                var field = elementor.panel.$el.find(
                    'select[data-setting="' + field_name + '"]'
                  ),
        ...
        if (
          ...
          elementorFrontend.config.settings.general = {
            elementor_stretched_section_container:
              TRX_ADDONS_STORAGE["elementor_stretched_section_container"],
          };
        }
        setTimeout(function () {
          trx_addons_once_resize = true;
    ...
    $document.on("action.scroll_trx_addons", function () {
      $scheme_watchers.each(function (idx) {
        ...
        if (typeof item_dom.getBoundingClientRect == "function") {
          ...
          item_cx =
            $window.scrollLeft() + item_offset.left + item_offset.width / 2;
          item_cy =
            $window.scrollTop() + item_offset.top + item_offset.height / 2;
        } else {
        ...
        $scheme_sections.each(function (idx2) {
          ...
          if (
            section_left < item_cx &&
            section_left + section.outerWidth() > item_cx &&
            section_top < item_cy &&
            section_top + section.outerHeight() > item_cy
          ) {
            var scheme = trx_addons_get_class_by_prefix(
              section.attr("class"),
              "scheme_"
            );
            if (!item.hasClass(scheme)) {
              item.attr(
                "class",
                trx_addons_chg_class_by_prefix(
                  item.attr("class"),
                  "scheme_",
                  scheme
                )
        ...
        if (!scheme_present) {
          item.attr(
            "class",
            trx_addons_chg_class_by_prefix(item.attr("class"), "scheme_", "")
          );
        }
      });
    });
  ...
  if ($body.hasClass("fixed_blocks_sticky")) {
    ...
    $document.on("action.resize_trx_addons", function () {
      ...
      if ($sections_wrap.length) {
        ...
        if (
          typeof TRX_ADDONS_STORAGE["elementor_stretched_section_container"] !=
          "undefined"
        ) {
          var $page_wrap = jQuery(
            TRX_ADDONS_STORAGE["elementor_stretched_section_container"]
          ).eq(0);
  ...
  $document.on("action.init_hidden_elements", function (e, cont) {
    var tabs = cont.hasClass("elementor-widget-tabs")
      ? cont
      : jQuery(".elementor-widget-tabs");
    if (
      tabs.hasClass("sc_tabs_open_on_hover_on") &&
      ...
      tabs
        ...
        .on(
          ...
          function (e) {
            ...
            if ($active.length) {
              var $tab_content = tabs.find(
                '.elementor-tab-content[data-tab="' + $active.data("tab") + '"]'
              );
        ...
        .on("click", ".elementor-tab-title", function (e) {
          var $self = jQuery(this);
          var $tab_content = tabs.find(
            '.elementor-tab-content[data-tab="' + $self.data("tab") + '"]'
          );
          setTimeout(
            function () {
  ...
  $document.on("action.activate_tab", function (e, cont) {
    cont
      .find(".elementor-widget-image-carousel .swiper-container")
      .each(function () {
  ...
  $document.on("action.prepare_popup_elements", function (e, cont, mfp) {
    ...
    items.each(function (idx) {
      var $self = jQuery(this),
        block =
          $self.hasClass("animation_type_block") ||
          $self.hasClass("animated-separate"),
        sc = block ? $self : $self.parents(".animated-separate"),
        params = sc.data("animation-settings") || sc.data("settings"),
        delay = trx_addons_elementor_animate_items_delay(
          params,
          sc,
          $self,
          idx,
      ...
      $self
        ...
        .removeClass(
          "animated " +
            animation +
            " " +
            trx_addons_elementor_animate_items_animation_revert_name(animation)
        );
      if (block || !mfp) {
  ...
  $document.on("action.close_popup_elements", function (e, cont) {
    ...
    items.each(function (idx) {
      ...
      var reverted = sc.data("animation-reverted"),
        animation = reverted
          ? reverted
          : trx_addons_elementor_animate_items_animation(params);
      $self.removeClass("animated " + animation);
      if (!reverted) {
        var revert_params =
          trx_addons_elementor_animate_items_animation_revert_params(
            sc,
            params,
            max_delay
          );
        sc.data(block ? "settings" : "animation-settings", revert_params).data(
          "animation-reverted",
          animation
        );
        setTimeout(function () {
          sc.data(block ? "settings" : "animation-settings", params).data(
            "animation-reverted",
            ""
          );
        }, max_delay);
  ...
  window.trx_addons_elementor_prepare_animate_items = function (force) {
    jQuery(".sc_section_toggle_on:not(.sc_section_toggle_inited)").each(
      function () {
        var sc = jQuery(this).addClass("sc_section_toggle_inited"),
          sc_cont = sc.find(">.elementor-container,>.e-con-inner"),
          cid = sc.data("model-cid"),
          params = cid
            ? trx_addons_elementor_get_settings_by_cid(cid, [
                "_animation",
                "animation",
              ])
            : sc.data("settings"),
          type = sc.hasClass("animation_type_block")
            ? "block"
        ...
        if (params) {
          for (var i in params) {
            if (
              i.substr(0, 10) == "_animation" ||
              i.substr(0, 9) == "animation"
            ) {
          ...
          sc.removeClass(
            "animated animation_type_" +
              type +
              " " +
              trx_addons_elementor_animate_items_animation(item_params) +
              (item_speed ? " " + item_speed : "") +
              (!sc.hasClass("elementor-element-edit-mode")
                ? " elementor-invisible trx_addons_invisible"
                : "")
          );
          if (!cid) {
            sc.attr("data-settings", JSON.stringify(params)).data(
              "settings",
              params
            );
          }
        ...
        if (id) {
          ...
          if ($link.length) {
            $link
              .addClass(
                "sc_section_toggle_state_" +
                  (sc.hasClass("sc_section_toggle_state_show")
                    ? "show"
                    : "hide")
              )
              .on("click", function (e) {
                e.preventDefault();
                e.stopImmediatePropagation();
                var state = sc.hasClass("sc_section_toggle_state_show")
                    ? "show"
                    : "hide",
                  action = state == "show" ? "hide" : "show",
                  max_delay = 0,
                  easing =
                    typeof jQuery.easing["easeOutSine"] != "undefined"
                      ? "easeOutSine"
                      : "linear";
                jQuery(this)
                ...
                if (action == "show") {
                  var h =
                    sc_cont.data("height") || trx_addons_window_height() * 0.4;
                  sc.addClass("sc_section_toggle_animated_show").animate(
                    ...
                    function () {
                      sc.css({ overflow: "hidden" })
                        .removeClass(
                          "sc_section_toggle_animated_show sc_section_toggle_state_" +
                            state
                        )
                        .addClass("sc_section_toggle_state_" + action);
                      ...
                      max_delay =
                        (sc.data("animation-delay") || 0) + item_duration;
                      setTimeout(function () {
                        sc.css({ height: "auto", overflow: "unset" });
                      }, max_delay);
                    }
                  );
                } else {
                  ...
                  setTimeout(function () {
                    sc.animate({ height: 0 }, 500, easing, function () {
                      sc.removeClass(
                        "sc_section_toggle_animated_hide sc_section_toggle_state_" +
                          state
                      ).addClass("sc_section_toggle_state_" + action);
                    });
                  }, max_delay);
            ...
            if (sc.hasClass("sc_section_toggle_close_on")) {
              sc.append(
                '<div class="sc_section_toggle_close_button trx_addons_button_close"><span class="trx_addons_button_close_icon"></span></div>'
              );
              sc.find(">.sc_section_toggle_close_button").on(
                "click",
                function () {
                  $link.trigger("click");
                }
    ...
    jQuery(
      '[class*="animation_type_"]:not(.animation_type_block)' +
        (!force ? ":not(.animated-separate)" : "")
    ).each(function () {
      var sc = jQuery(this),
        is_section =
          sc.hasClass("elementor-section") ||
          sc.hasClass("elementor-container") ||
          sc.hasClass("elementor-row"),
      ...
      if (sc_name) {
        ...
        sc_name =
          is_container && sc.find(">.e-con-inner>.e-con,>.e-con").length > 1
      ...
      if (
        (!is_section || sc_name.indexOf(".elementor-column") < 0) &&
        (!is_container || sc_name.indexOf(".e-con") < 0)
      ) {
        sc_name +=
          sc_name && TRX_ADDONS_STORAGE["elementor_animate_items"]
      ...
      for (var i in params) {
        if (i.substr(0, 10) == "_animation" || i.substr(0, 9) == "animation") {
          item_params[(i.substr(0, 9) == "animation" ? "_" : "") + i] =
            params[i];
          delete params[i];
        }
      }
      sc.removeClass(
        "elementor-invisible trx_addons_invisible animated " +
          trx_addons_elementor_animate_items_animation(item_params) +
      ...
      sc.attr("data-animation-settings", JSON.stringify(item_params)).data(
        "animation-settings",
        item_params
      );
      items.each(function (idx) {
        var item = jQuery(this);
        if (
          item.hasClass("sc_item_title") ||
          item.hasClass("elementor-heading-title")
        ) {
          item.html(
            trx_addons_wrap_words(
              item.html(),
              '<span class="sc_item_animated_block elementor-invisible trx_addons_invisible animated-item' +
                (item_speed ? " " + item_speed : "") +
                '">',
              "</span>"
            )
        ...
        for (var s in settings) {
          for (var i = 0; i < keys.length; i++) {
            if (s.indexOf(keys[i]) === 0) {
              if (
                typeof settings[s] == "object" &&
                settings[s].hasOwnProperty("models")
              ) {
                var tmp = [];
  ...
  function trx_addons_elementor_animate_items_scroll(force) {
    ...
    $animated_items.each(function (idx) {
      ...
      if (
        !force &&
        (item.hasClass("animated") || item.hasClass("wait-for-animation"))
      )
        return;
      ...
      if (item_top + 50 < window_top + window_height) {
        ...
        trx_addons_elementor_animate_item(
          item,
          cnt,
          $animated_items.length - idx + cnt,
          force || !need_animation
        );
        if (need_animation) {
          cnt++;
        }
  ...
  function trx_addons_elementor_animate_item(item, idx, total, force) {
    ...
    if (
      (sc.hasClass("elementor-container") || sc.hasClass("e-con-inner")) &&
      sc.parent().hasClass("sc_section_toggle_state_hide")
    ) {
      return;
    }
    var item_params = sc.data(block ? "settings" : "animation-settings"),
      item_delay = trx_addons_elementor_animate_items_delay(
        item_params,
        sc,
        item,
        idx,
  ...
  function trx_addons_elementor_animate_items_delay(
    params,
    sc,
    item,
    idx,
    ...
    var delay = sc.hasClass("animation_type_block")
      ? params && params._animation_delay
        ? params._animation_delay
        : 0
      : sc.hasClass("animation_type_sequental")
      ? (params && params._animation_delay ? params._animation_delay : 150) *
        (sc.data("animation-reverted")
          ? item.hasClass("menu-item")
            ? item.siblings(".menu-item").length + 1 - item.index()
            : Math.min(8, total - idx)
          : item.hasClass("menu-item")
          ? item.index()
          : Math.min(8, idx))
      : trx_addons_random(
          0,
          params && params._animation_delay ? params._animation_delay : 1500
        );
    return delay;
  }
  ...
  function trx_addons_elementor_animate_items_animation_revert_params(
    sc,
    params,
    max_delay
  ) {
    ...
    if (params) {
      ...
      if (typeof revert_params["_animation" + device] != "undefined") {
        revert_params["_animation" + device] =
          trx_addons_elementor_animate_items_animation_revert_name(
            revert_params["_animation" + device]
          );
      } else if (typeof revert_params["_animation"] != "undefined") {
        revert_params["_animation"] =
          trx_addons_elementor_animate_items_animation_revert_name(
            revert_params["_animation"]
          );
      } else if (typeof revert_params["animation" + device] != "undefined") {
        revert_params["animation" + device] =
          trx_addons_elementor_animate_items_animation_revert_name(
            revert_params["animation" + device]
          );
      } else if (typeof revert_params["animation"] != "undefined") {
        revert_params["animation"] =
          trx_addons_elementor_animate_items_animation_revert_name(
            revert_params["animation"]
          );
      }
  ...
  function trx_addons_elementor_add_bg_text_in_row(row) {
    ...
    if (data["bg_text"]) {
      ...
      var chars = "",
        ...
        ch;
      var delimiter_image =
        typeof data["bg_text_delimiter_image"] == "object"
          ? data["bg_text_delimiter_image"]["url"]
          : data["bg_text_delimiter_image"];
      var delimiter_svg = data["bg_text_delimiter_svg"] || "";
      var delimiter_icon = !trx_addons_is_off(data["bg_text_delimiter_icon"])
        ? data["bg_text_delimiter_icon"]
        : "";
          ...
          if (!in_tag) {
            if (ch == "<") {
              ...
              if (ch == "&") {
                ...
                if (ch == ";") {
                  chars +=
                    '<span class="trx_addons_bg_text_char">' + amp + "</span>";
                  in_amp = false;
                  amp = "";
                }
              } else {
                chars +=
                  '<span class="trx_addons_bg_text_char">' +
                  (ch == " " ? "&nbsp;" : ch) +
                  "</span>";
              }
            }
      ...
      var marquee_speed =
          typeof data["bg_text_marquee"] == "object"
            ? data["bg_text_marquee"]["size"]
              ? data["bg_text_marquee"]["size"]
              : 0
            : data["bg_text_marquee"],
        marquee_dir =
          typeof data["bg_text_reverse"] != "undefined"
            ...
            : -1,
        overlay =
          typeof data["bg_text_overlay"] == "object"
      ...
      if (marquee_speed > 0 && (delimiter_icon || delimiter_image)) {
        chars +=
          ...
          (!delimiter_image && delimiter_icon
            ? " trx_addons_bg_text_delimiter_icon " + delimiter_icon
            : " trx_addons_bg_text_delimiter_image") +
          (data["bg_text_delimiter_rotation"] > 0
            ? " trx_addons_bg_text_delimiter_rotation"
            : "") +
          '">' +
          (delimiter_image
            ? delimiter_svg
      ...
      row_cont.prepend(
        '<div class="trx_addons_bg_text' +
          ...
          ((!rtl && marquee_dir > 0) || (rtl && marquee_dir < 0)
            ? " trx_addons_marquee_reverse"
            : "") +
          '">' +
          '<div class="trx_addons_bg_text_inner' +
          " trx_addons_bg_text_effect_" +
          data["bg_text_effect"] +
          (marquee_speed > 0
            ? " trx_addons_marquee_element" +
              (data["bg_text_left"]["size"] != 0
                ? " trx_addons_marquee_element_with_offset"
                : "")
            : "") +
          (cid == ""
            ? " trx_addons_show_on_scroll"
            : " trx_addons_showed_on_scroll trx_addons_in_preview_mode") +
          '"' +
          ">" +
          chars +
          "</div>" +
          ...
          marquee_chars = Math.min(
            ...
            data["bg_text_effect"] == "none"
              ? trx_addons_clear_tags(data["bg_text"]).length +
                  (delimiter_icon || delimiter_image ? 1 : 0)
              : marquee_wrap.find(".trx_addons_bg_text_char").length
          );
        setTimeout(
          function () {
            trx_addons_elementor_marquee_bg_text(
              marquee_wrap,
              marquee_dir,
              marquee_speed,
              true,
              data["bg_text_effect"],
              data["bg_text_marquee_hover"],
              data["bg_text_accelerate"]
            );
          },
  ...
  function trx_addons_elementor_marquee_bg_text(
    marquee_wrap,
    marquee_dir,
    marquee_speed,
    start,
    ...
    var elements = marquee_wrap.find(".trx_addons_marquee_element"),
      ...
      divider =
        divider_min +
        ((mpw - mpw_min) / (mpw_max - mpw_min)) * (divider_max - divider_min),
      time_per_pixel = (50 - Math.min(15, marquee_speed) * 3) / divider,
      with_offset = elements
        .eq(0)
        .hasClass("trx_addons_marquee_element_with_offset"),
      coeff_max = trx_addons_apply_filters(
        "trx_addons_filter_bg_text_marquee_coeff_max",
        Math.max(2, Math.ceil(mpw / mw))
      ),
      coeff =
        with_offset && marquee_wrap.data("marquee-loop")
          ? Math.min(marquee_wrap.data("marquee-loop") + 1, coeff_max)
          : 1;
    if (
      effect == "none" &&
      elements.eq(0).hasClass("trx_addons_show_on_scroll") &&
      !elements.eq(0).hasClass("trx_addons_showed_on_scroll")
    ) {
      elements
        .eq(0)
        .removeClass("trx_addons_show_on_scroll")
    ...
    if (elements.eq(0).hasClass("trx_addons_showed_on_scroll")) {
      if (start) {
        for (
          var i = 1;
          i < Math.ceil((mpw + mw * (with_offset ? coeff_max + 1 : 2)) / mw);
          i++
        ) {
      ...
      var delimiters = marquee_wrap
          .find(".trx_addons_bg_text_delimiter_rotation")
          .css({
        ...
        delimiters_loop =
          delimiters.length > 0
            ? Math.max(
                1,
                Math.ceil(mpw / (delimiters.eq(0).outerHeight() * Math.PI) / 2)
              )
            : 0;
      elements.each(function (idx) {
        ...
        $self
          .data(
            "tweenmax-object",
            TweenMax.to(elements.eq(idx), mw * time_per_pixel, {
              overwrite: true,
              ...
              onComplete: function () {
                if (idx == elements.length - 1) {
                  marquee_wrap.data(
                    "marquee-loop",
                    (marquee_wrap.data("marquee-loop") || 0) + 1
                  );
                  elements.each(function (idx2) {
                    TweenMax.to(elements.eq(idx2), 0, {
                      overwrite: true,
                      x: with_offset
                        ? mw * Math.min(coeff, coeff_max - 1) * marquee_dir
                        : 0,
                      y: 0,
                      ease: Power0.easeNone,
                    });
          ...
          .data(
            ...
            delimiters_loop === 0
              ? null
              : TweenMax.to(delimiters.eq(idx), mw * time_per_pixel, {
                  overwrite: true,
    ...
    if (accelerate) {
      var wheel_busy = false,
        ...
        wheel_handler = function (e) {
          ...
          elements.eq(0).data(
            "tweenmax-accelerate",
            TweenMax.to(wheel_accelerate, 2.0, {
              overwrite: true,
              value: 15,
              ease: Power4.easeOut,
              onUpdate: function () {
      ...
      window.addEventListener("mousewheel", wheel_handler, { passive: false });
...
(function () {
  ...
  $document.on(
    "action.got_ajax_response action.init_hidden_elements",
    function (e) {
      setTimeout(function () {
        var items = jQuery(
          ".trx_addons_parallax_layers:not(.trx_addons_parallax_layers_inited)"
        );
        if (items.length > 0) {
          items.each(function () {
            var layers = new trx_addons_parallax(
              jQuery(this).addClass("trx_addons_parallax_layers_inited"),
              "layers"
            );
            if (layers.init()) {
        ...
        if (items.length > 0) {
          items.each(function () {
            ...
            if ($self.hasClass("sc_parallax_text_separate_on")) {
              ...
              var blocks = new trx_addons_parallax(
  ...
  function parallax_text_prepare($init_obj, settings) {
    ...
    if (
      !settings ||
      settings.length === 0 ||
      ["chars", "words"].indexOf(settings[0].text) == -1 ||
      !$init_obj.data("element_type")
    ) {
      return false;
    }
    var sc = (
        $init_obj.data("element_type") == "widget"
          ? $init_obj.data("widget_type")
          : $init_obj.data("element_type")
      ).split(".")[0],
        ...
        if (settings[0].text_separate) {
          $inner_obj.each(function (idx) {
            ...
            if (params.lag > 0) {
              params.lag =
                trx_addons_window_width() <
                TRX_ADDONS_STORAGE["mobile_breakpoint_scroll_lag_off"]
                  ? 0
                  : params.lag * (idx + 1);
            }
            $inner_obj
              .eq(idx)
              .attr("data-parallax-params", JSON.stringify(params));
          });
        }
      }
    }
  ...
  function get_editor_settings($target, init_type) {
    ...
    function get_section_data(idx, obj) {
      if (0 < Object.keys(section_data).length) {
        ...
        obj.attributes &&
        obj.attributes.elements &&
        obj.attributes.elements.models
      ) {
    ...
    if (
      init_type == "layers" &&
      section_data.hasOwnProperty("parallax_blocks")
    ) {
      jQuery.each(
        section_data["parallax_blocks"].models,
        function (index, obj) {
      ...
      init_type == "blocks" &&
      ((section_data.hasOwnProperty("parallax") &&
        section_data.parallax == "parallax") ||
        (section_data.hasOwnProperty("parallax_mouse") &&
          section_data.parallax_mouse == "mouse"))
    ) {
      var parallax_on =
          section_data.hasOwnProperty("parallax") &&
          section_data.parallax == "parallax",
      ...
      var data = {
        ...
        type:
          parallax_on && section_data.hasOwnProperty("parallax_type")
            ? section_data.parallax_type
            : "object",
        flow:
          parallax_on && section_data.hasOwnProperty("parallax_flow")
            ? section_data.parallax_flow
            : "default",
        crop:
          parallax_on && section_data.hasOwnProperty("parallax_crop")
            ? section_data.parallax_crop
            : "none",
        duration:
          parallax_on && section_data.hasOwnProperty("parallax_duration")
            ? section_data.parallax_duration.size
            : 1,
        delay:
          parallax_on && section_data.hasOwnProperty("parallax_delay")
            ? section_data.parallax_delay.size
            : 0,
        squeeze:
          parallax_on && section_data.hasOwnProperty("parallax_squeeze")
            ? section_data.parallax_squeeze.size
            : 1,
        ease:
          parallax_on && section_data.hasOwnProperty("parallax_ease")
            ? section_data.parallax_ease
            : "power2",
        text:
          parallax_on && section_data.hasOwnProperty("parallax_text")
            ? section_data.parallax_text
            : "block",
        text_separate:
          parallax_on && section_data.hasOwnProperty("parallax_text_separate")
            ? section_data.parallax_text_separate
            : 0,
        text_wrap:
          parallax_on && section_data.hasOwnProperty("parallax_text_wrap")
            ? 1
            : 0,
        mouse: mouse_on ? 1 : 0,
        mouse_type:
          mouse_on && section_data.hasOwnProperty("parallax_mouse_type")
            ? section_data.parallax_mouse_type
            : "transform3d",
        mouse_tilt_amount:
          mouse_on && section_data.hasOwnProperty("parallax_mouse_tilt_amount")
            ? section_data.parallax_mouse_tilt_amount.size
            : 70,
        mouse_speed:
          mouse_on && section_data.hasOwnProperty("parallax_mouse_speed")
            ? section_data.parallax_mouse_speed.size
            : 10,
        mouse_z:
          mouse_on && section_data.hasOwnProperty("parallax_mouse_z")
            ? section_data.parallax_mouse_z.size
            : "",
        mouse_handler:
          mouse_on && section_data.hasOwnProperty("parallax_mouse_handler")
            ? section_data.parallax_mouse_handler
            : "row",
      };
      var suffix_prev = "",
        suffix = "",
      ...
      for (var resp in TRX_ADDONS_STORAGE["elementor_breakpoints"]) {
        ...
        for (var param in bp_params) {
          ...
          if (suffix && !section_data.hasOwnProperty("parallax_" + param_name))
            continue;
          data[param_name] =
            section_data.hasOwnProperty("parallax_" + param_name) &&
            section_data["parallax_" + param_name]["size"] !== ""
              ...
              : data[param_prev];
          if (defa["unit"]) {
            data[param_name + "_unit"] =
              section_data.hasOwnProperty("parallax_" + param_name) &&
              section_data["parallax_" + param_name].hasOwnProperty("unit") &&
              section_data["parallax_" + param_name]["size"] !== ""
                ? section_data["parallax_" + param_name]["unit"]
                : !suffix
                ? defa["unit"]
                : data[param_prev + "_unit"];
          }
          if (!suffix) {
        ...
        if (data["x_end" + suffix] == 0)
          data["x_end" + suffix + "_unit"] = data["x_start" + suffix + "_unit"];
        else if (data["x_start" + suffix] == 0)
          data["x_start" + suffix + "_unit"] = data["x_end" + suffix + "_unit"];
        if (data["y_end" + suffix] == 0)
          data["y_end" + suffix + "_unit"] = data["y_start" + suffix + "_unit"];
        else if (data["y_start" + suffix] == 0)
          data["y_start" + suffix + "_unit"] = data["y_end" + suffix + "_unit"];
        suffix_prev = suffix;
      }
  ...
  window.trx_addons_parallax = function ($target, init_type) {
    ...
    self.init = function () {
      ...
      if (settings[0].mouse == 1) {
        ...
        var layout_data = {
          ...
          speed:
            2 *
            ((settings[0].mouse_speed ? settings[0].mouse_speed : 10) / 100),
      ...
      if (init_type == "layers" || settings[0].mouse == 1) {
        if (settings[0].mouse_handler == "window") {
          ...
          $targetLayers = jQuery(
            trx_addons_apply_filters(
              "trx_addons_filter_page_wrap_class",
              TRX_ADDONS_STORAGE["page_wrap_class"]
                ? TRX_ADDONS_STORAGE["page_wrap_class"]
                : ".page_wrap",
              "elementor-parallax"
            )
          ...
          settings[0].mouse_handler &&
          ".#".indexOf(settings[0].mouse_handler.substring(0, 1)) != -1
        ) {
      ...
      if (parallax_type.indexOf("layers") >= 0) {
        ...
        if (animation_allowed) {
          if (!$targetLayers.attr("data-mousemove-" + target_id)) {
            ...
            $targetLayers.on(
              "mousemove.trx_addons_parallax",
              self.mouse_move_handler
            );
            $targetLayers.on(
              "mouseleave.trx_addons_parallax",
              self.mouse_leave_handler
            );
          }
          if (motion_list.length > 0) {
            $targetLayers.attr(
              "data-motionmove-" + target_id,
      ...
      if (parallax_type.indexOf("blocks") >= 0) {
        ...
        settings[0].hscroller = settings[0].hsection.length
          ? settings[0].hsection.parents(".sc_hscroll_scroller")
          : false;
      ...
      if (!GET_SIZES_ON_SCROLL) {
        ...
        $document.on(
          "action.init_hidden_elements action.hscroll_inited",
          self.get_blocks_sizes
        );
        $document.on(
          "action.sc_layouts_row_fixed_on action.sc_layouts_row_fixed_off",
          self.get_blocks_sizes
        );
      ...
      if (animation_allowed) {
        ...
        $document.on(
          "action.resize_trx_addons action.scroll_trx_addons action.hscroll_inited",
          self.scroll_handler
        );
        $document.on(
          "action.resize_trx_addons action.scroll_trx_addons action.hscroll_inited action.init_hidden_elements action.sc_layouts_row_fixed_on action.sc_layouts_row_fixed_off",
          on_start_init
        );
      }
      return true;
    ...
    self.get_blocks_sizes = function () {
      jQuery.each(scroll_list, function (index, block) {
        var hscroller_offset = block.hsection.length
          ? block.hscroller.data("hscroll-offset") || 0
          : 0;
        var hsection_offset = block.hsection.length
          ? block.hsection.data("hscroll-section-offset") || 0
          : 0;
        ...
        if (block.flow == "sticky") {
          ...
          block.sizes.parent_height = $parent.length
            ? $parent.outerHeight()
            : 0;
          block.sizes.parent_top = $parent.length
            ? $parent.offset().top + hscroller_offset
            : 0;
        }
      });
    };
      ...
      jQuery.each(settings, function (index, block) {
        var image = block["image"].url,
          ...
          anim_prop = block["animation_prop"]
            ? ["scroll", "motion"].indexOf(type) != -1 &&
              block["animation_prop"] != "background"
              ? "transform"
          ...
          tilt_amount = block["mouse_tilt_amount"]
            ? block["mouse_tilt_amount"].size
            : 70,
          mouse_handler = block["mouse_handler"]
            ? block["mouse_handler"]
            : "row",
          bg_parallax =
            block["bg_parallax"] &&
            type == "scroll" &&
            anim_prop == "background"
              ? block["bg_parallax"]
              : false,
          $layout = null;
        if (bg_parallax) {
        ...
        if ("" !== image || "none" !== type) {
          var layout_init = { "z-index": z_index };
          if ("none" === type) {
            layout_init["left"] = left + "%";
            layout_init["top"] = top + "%";
          }
          $layout = jQuery(
            '<div class="sc_parallax_block' +
              " sc_parallax_block_type_" +
              type +
              " sc_parallax_block_animation_" +
              (bg_parallax ? "bg_parallax" : anim_prop) +
              (is_safari ? " is-safari" : "") +
              ("MacIntel" == platform ? " is-mac" : "") +
              (typeof block["class"] !== "undefined" && block["class"] != ""
                ? " " + block["class"]
                : "") +
              '">' +
          ...
          if ("scroll" === type) {
            layout_data.hsection = layout_data.selector.parents(
              ".sc_hscroll_section"
            );
            layout_data.hscroller = layout_data.hsection.length
              ? layout_data.hsection.parents(".sc_hscroll_scroller")
              : false;
    ...
    self.get_block_params = function (block) {
      ...
      if (params.flow == "in_out") {
        var w_top = wst + wh * (1 - params.range_end / 100),
          ...
          obj_top =
            params.sizes.top +
            (ww < TRX_ADDONS_STORAGE["mobile_breakpoint_scroll_lag_off"]
              ? 0
              : params.lag),
    ...
    self.scroll_update = function (force) {
      ...
      jQuery.each(scroll_list, function (index, block) {
        if (parallax_type.indexOf("layers") >= 0) {
          if (
            (!block.bg_parallax || block.prop != "background") &&
        ...
        if (parallax_type.indexOf("blocks") >= 0) {
          ...
          var w_top = wst + wh * (1 - params.range_end / 100),
            ...
            parent_bottom =
              params.flow == "sticky"
                ? parent_top +
                  params.sizes.parent_height * (1 - params.sticky_offset / 100)
                : 0;
          var entrance_complete = obj.hasClass("sc_parallax_entrance_complete"),
            bottom_delta =
              params.flow == "entrance" && params.range_start == 0
          ...
          if (
            ...
            if (params.flow == "entrance") {
              ...
              var obj_anchor =
                obj_top +
                Math.round((obj_height * (100 - params.range_start)) / 100);
              ...
              delta = Math.max(
                1,
                (wh * (params.range_end - params.range_start)) / 100
              );
              shift =
                w_bottom - (params.in_out_state == "in" ? obj_top : obj_bottom);
            } else {
              delta = Math.max(
                1,
                (wh * (params.range_end - params.range_start)) / 100 +
            ...
            var x_start_in_px =
                params.x_start_unit == "px" ||
                params.x_start_unit == params.x_end_unit
                  ? params.x_start
                  : trx_addons_units2px(
                      params.x_start + params.x_start_unit,
                      obj,
                      "x"
                    ),
              x_end_in_px =
                params.x_end_unit == "px" ||
                params.x_start_unit == params.x_end_unit
                  ? params.x_end
                  : trx_addons_units2px(
                      params.x_end + params.x_end_unit,
                      obj,
                      "x"
                    ),
              y_start_in_px =
                params.y_start_unit == "px" ||
                params.y_start_unit == params.y_end_unit
                  ? params.y_start
                  : trx_addons_units2px(
                      params.y_start + params.y_start_unit,
                      obj,
                      "y"
                    ),
              y_end_in_px =
                params.y_end_unit == "px" ||
                params.y_start_unit == params.y_end_unit
                  ? params.y_end
                  : trx_addons_units2px(
                      params.y_end + params.y_end_unit,
                      obj,
                      "y"
                    ),
              step_x =
                x_start_in_px != x_end_in_px
                  ? (x_end_in_px - x_start_in_px) / delta
                  : 0,
              step_y =
                y_start_in_px != y_end_in_px
                  ? (y_end_in_px - y_start_in_px) / delta
                  : 0,
              step_scale =
                params.scale_start != params.scale_end
                  ? (params.scale_end - params.scale_start) / 100 / delta
                  : 0,
              step_rotate =
                params.rotate_start != params.rotate_end
                  ? (params.rotate_end - params.rotate_start) / delta
                  : 0,
              step_opacity =
                params.opacity_start != params.opacity_end
                  ? (params.opacity_end - params.opacity_start) / delta
                  : 0,
              step_crop =
                !trx_addons_is_off(params.crop) &&
                params.crop_start != params.crop_end
                  ? (params.crop_end - params.crop_start) / delta
                  : 0;
            var scroller_init = {
                overwrite: true,
            ...
            if (step_x !== 0) {
              ...
              if (
                (x_start_in_px < x_end_in_px && val < x_start_in_px) ||
                (x_start_in_px > x_end_in_px && val > x_start_in_px)
              ) {
                val = x_start_in_px;
              }
              if (
                (x_start_in_px < x_end_in_px && val > x_end_in_px) ||
                (x_start_in_px > x_end_in_px && val < x_end_in_px)
              ) {
                val = x_end_in_px;
              }
            ...
            if (step_y !== 0) {
              ...
              if (
                (y_start_in_px < y_end_in_px && val < y_start_in_px) ||
                (y_start_in_px > y_end_in_px && val > y_start_in_px)
              ) {
                val = y_start_in_px;
              }
              if (
                (y_start_in_px < y_end_in_px && val > y_end_in_px) ||
                (y_start_in_px > y_end_in_px && val < y_end_in_px)
              ) {
                val = y_end_in_px;
              }
            ...
            if (step_rotate !== 0) {
              val = trx_addons_round_number(
                params.rotate_start + shift * step_rotate,
                2
              );
              if (
                (params.rotate_start < params.rotate_end &&
            ...
            if (step_scale !== 0) {
              val = trx_addons_round_number(
                params.scale_start / 100 + shift * step_scale,
                2
              );
              if (
                (params.scale_start < params.scale_end &&
            ...
            if (step_opacity !== 0) {
              val = trx_addons_round_number(
                params.opacity_start + shift * step_opacity,
                2
              );
              if (
                (params.opacity_start < params.opacity_end &&
            ...
            if (step_crop !== 0) {
              val = trx_addons_round_number(
                params.crop_start + shift * step_crop,
                2
              );
              if (
                (params.crop_start < params.crop_end &&
                  val < params.crop_start) ||
                (params.crop_start > params.crop_end && val > params.crop_start)
              ) {
                val = params.crop_start;
              }
              if (
                (params.crop_start < params.crop_end &&
                  val > params.crop_end) ||
                (params.crop_start > params.crop_end && val < params.crop_end)
              ) {
                val = params.crop_end;
              }
              ...
              scroller_init.onUpdate = function (t) {
                obj.css(
                  "clip-path",
                  self.clip_path(obj.prop("crop"), params.crop, obj)
                );
              };
            }
            obj.data("trx-parallax-scroller-init", scroller_init);
            if (
              ["chars", "words"].indexOf(params.text) != -1 &&
              obj.data("element_type") !== undefined
            ) {
              ...
              var animate = function () {
                obj.each(function (idx) {
                  if (idx === 0 || set_start_position) {
                    TweenMax.to(
                      obj.eq(idx),
                      set_start_position ? 0 : params.duration,
                      scroller_init
                    );
                    if (set_start_position) {
                      obj
                        .eq(idx)
                        .addClass("trx_addons_parallax_blocks_on_start");
                    }
                  } else {
                    setTimeout(function () {
                      TweenMax.to(obj.eq(idx), params.duration, scroller_init);
                    }, (params.text == "chars" ? 75 : 250) *
                      idx *
                      params.squeeze);
                  }
                });
    ...
    self.clip_path = function (val, type, obj) {
      ...
      if (type == "circle") {
        ...
        clip =
          "ellipse( " + Math.min(100, val * 1.2) + "% " + val + "% at center)";
      } else if (type == "ellipse_ver") {
        clip =
          "ellipse(" + val + "% " + Math.min(100, val * 2.5) + "% at center)";
      } else {
        var border_radius =
          obj.data("trx-addons-clip-border-radius") === undefined
        ...
        var x_divider =
          ["wipe_out_vertical", "box_center", "box_top", "box_bottom"].indexOf(
            type
          ) != -1
            ? 2
            : 1;
    ...
    self.mouse_move_handler = function (e) {
      if (tw === 0) {
        ...
        th =
          ["window", "content"].indexOf($targetLayers.data("mouse-handler")) !=
          -1
      ...
      dx = e.clientX - tl - cx;
      dy =
        ["window", "content"].indexOf($targetLayers.data("mouse-handler")) != -1
          ? e.clientY - cy
          : e.clientY + wst - tt - cy;
      tx = -1 * (dx / cx);
      ty = -1 * (dy / cy);
      ...
      jQuery.each(mouse_list, function (index, block) {
        ...
        if (scroller_init) {
          x =
            x * 1 +
            trx_addons_units2px(
              (scroller_init.css ? scroller_init.css.x : scroller_init.x) || 0,
              block,
              "x"
            );
          y =
            y * 1 +
            trx_addons_units2px(
              (scroller_init.css ? scroller_init.css.y : scroller_init.y) || 0,
              block,
              "y"
            );
        }
    ...
    self.mouse_move_update = function (index, block, time, ease) {
      ...
      if (block.prop == "background") {
        ...
        var m = block.tilt_amount > 0 ? block.tilt_amount : 70,
          k =
            ["window", "content"].indexOf(
              $targetLayers.data("mouse-handler")
            ) != -1
              ? 2
              : 4;
    ...
    self.motion_move_handler = function () {
      ...
      jQuery.each(motion_list, function (index, block) {
        var fi,
          delta =
            ((motion_period * motion_step) % (block["motion_time"] * 1000)) /
            (block["motion_time"] * 1000),
        ...
        if (block["motion_dir"] == "round") {
          ...
          dx =
            block["motion_dir"] == "vertical" ? 0 : (tw / 2) * Math.cos(angle);
          dy =
            block["motion_dir"] == "horizontal"
              ? 0
              : (th / 2) * Math.sin(angle);
        }
        tx = -1 * (dx / cx);
        ...
        if (block["motion_dir"] == "random") {
          ...
          self.mouse_move_update(
            ...
            block,
            block["motion_time"],
            block["motion_dir"] == "round" ? Power0.easeNone : Power2.easeOut
          );
        }
      });
      motion_step++;
  ...
  window.trx_addons_bg_parallax = function (clip) {
    ...
    window.addEventListener(
      "resize",
      bg_parallax_resize.bind(null, parallaxDetails)
    );
...
jQuery(document).on("action.ready_trx_addons", function () {
  ...
  var USE_OBSERVERS =
    trx_addons_apply_filters(
      "trx_addons_filter_use_observers_to_fix_rows",
      true
  ...
  if (
    !TRX_ADDONS_STORAGE["pagebuilder_preview_mode"] &&
    !$body.hasClass("sc_layouts_row_fixed_inited")
  ) {
      ...
      rows.each(function (idx) {
        ...
        if (row.hasClass("sc_layouts_row_hide_unfixed")) {
          if (!row.prev().hasClass("sc_layouts_row_unfixed_placeholder")) {
            row.before(
              '<div class="sc_layouts_row_unfixed_placeholder">' +
                (USE_OBSERVERS
                  ? '<div class="sc_layouts_row_fixed_marker_on"></div>'
                  : "") +
                "</div>"
            );
          }
        ...
        if (!row.next().hasClass("sc_layouts_row_fixed_placeholder")) {
          row.after(
            '<div class="sc_layouts_row_fixed_placeholder" style="background-color:' +
              ...
              ';"><div class="sc_layouts_row_fixed_marker_off"></div></div>'
          );
        ...
        if (
          row.find(
            trx_addons_apply_filters(
              "trx_addons_filter_fullscreen_elements",
              ".search_style_fullscreen,.search_modern"
            )
          ).length > 0
        ) {
      ...
      if (USE_OBSERVERS) {
        var fixed_rows_observe = function () {
          rows.each(function (idx) {
            ...
            var marker_off = row
              .next()
              .find(">.sc_layouts_row_fixed_marker_off");
            ...
            if (
              trx_addons_window_width() <
                TRX_ADDONS_STORAGE["mobile_breakpoint_fixedrows_off"] &&
              !row.hasClass("sc_layouts_row_fixed_always")
            ) {
              return;
            }
            var row_height = row.hasClass("sc_layouts_row_hide_unfixed")
              ? 0
              : row.outerHeight();
              ...
              function (entries) {
                ...
                entries.forEach(function (entry) {
                  ...
                  if (parent.hasClass("sc_layouts_row_unfixed_placeholder")) {
                    ...
                    parent.hasClass("sc_layouts_row_fixed_placeholder")
                  ) {
                    row = row.prev();
                  }
                  var animation_off_timeout =
                    trx_addons_cpt_layouts_fix_rows_off_timeout(delay);
                  if (
                    row.hasClass("sc_layouts_row_fixed_on") &&
                    entry.isIntersecting
                  ) {
                    if (
                      time - last_fixed_time > animation_off_timeout &&
            ...
            var marker = row.hasClass("sc_layouts_row_fixed_on")
              ? marker_off
              : marker_on;
  ...
  function trx_addons_cpt_layouts_fix_rows_delay(row) {
    return trx_addons_apply_filters(
      "trx_addons_filter_fixed_rows_delay",
  ...
  function trx_addons_cpt_layouts_hide_fixed_rows() {
    if (
      TRX_ADDONS_STORAGE["hide_fixed_rows"] > 0 &&
      !window.trx_addons_document_animate_to_busy
    ) {
      var scroll_delta = 50;
      ...
      if (last_scroll_offset >= 0) {
        ...
        if (scroll_offset > last_scroll_offset + scroll_delta) {
          if (
            scroll_offset >
              trx_addons_window_height() *
                (rows_delayed.length > 0 ? 1.5 : 0.6667) &&
            !$body.hasClass("hide_fixed_rows")
          ) {
        ...
        if (event) {
          $document.trigger("action.sc_layouts_row_fixed_" + event, [
            rows.filter(".sc_layouts_row_fixed_on"),
          ]);
        }
      } else {
        last_scroll_offset = scroll_offset;
  ...
  function trx_addons_cpt_layouts_fix_rows(args) {
    ...
    rows.each(function (idx) {
      ...
      if (
        (row.css("display") == "none" || h === 0) &&
        !row.hasClass("sc_layouts_row_hide_unfixed")
      ) {
        placeholder.height(0);
        return;
      }
      var ph = row.hasClass("sc_layouts_row_fixed_on")
        ? placeholder.outerHeight()
        : 0;
      var row_unfixed_placeholder = row.hasClass("sc_layouts_row_hide_unfixed")
        ? row.prev()
        : false;
      var delay = trx_addons_cpt_layouts_fix_rows_delay(row);
      var animation_off_timeout =
      ...
      if (
        ...
        (!force_state &&
          (scroll_offset + rows_offset < offset + delay || h < ph))
      ) {
            ...
            row
              .removeClass(
                "sc_layouts_row_fixed_on" +
                  (animation_off_timeout > 0
                    ? " sc_layouts_row_fixed_animation_off"
                    : "")
              )
              .css({ top: "auto" });
...
(function () {
  ...
  var $window = jQuery(window),
    $document = jQuery(document);
  var init_attempts = 0;
  var init_hidden_elements_after_all_sliders_inited = trx_addons_throttle(
    function () {
  ...
  function trx_addons_init_sliders(e, container) {
    ...
    if (window.Swiper) {
      container.find(".sc_slider_controller:not(.inited)").each(function () {
        ...
        var controller_html = "";
        slider_cont
          .find("> .slider-wrapper > .swiper-slide")
          .each(function (idx) {
            ...
            var title =
              controller_style.indexOf("titles") == -1
                ? ""
                : slide.data("title")
                ? slide.data("title")
        ...
        controller.html(
          trx_addons_apply_filters(
            ...
            '<div id="' +
              ...
              (controller_direction == "vertical" ? "vertical" : "horizontal") +
              '"' +
              ' data-slides-min-width="' +
              trx_addons_apply_filters(
                "trx_addons_filter_slider_controller_slide_width",
                150
              ) +
              '"' +
      ...
      container.find(".sc_slider_controls:not(.inited)").each(function () {
        ...
        var s =
            ...
            typeof TRX_ADDONS_STORAGE["swipers"][slider_id] != "undefined"
        ...
        if (pagination_style == "thumbs") {
          slides.each(function (idx) {
            ...
            html += trx_addons_apply_filters(
              ...
              '<span class="slider_pagination_button_wrap swiper-pagination-button-wrap" style="width: ' +
                Math.round(100 / total, 2) +
                '%;">' +
          ...
          controls
            .find(".slider_pagination_wrap")
            .html(
              trx_addons_apply_filters(
                "trx_addons_filter_slider_controls_html_thumbs",
                html,
                controls
              )
            );
          ...
          controls
            ...
            .html(
              trx_addons_apply_filters(
                "trx_addons_filter_slider_controls_html_fraction",
                '<span class="slider_pagination_current swiper-pagination-current">1</span>' +
                  "/" +
                  '<span class="slider_pagination_total swiper-pagination-total">' +
                  total +
                  "</span>",
                controls
              )
          ...
          slides.each(function (idx) {
            ...
            html += trx_addons_apply_filters(
              ...
              '<span class="slider_pagination_bullet swiper-pagination-bullet" data-slide-number="' +
                (s ? jQuery(this).data("slide-number") : idx) +
                '"></span>',
              idx,
              controls
            );
        ...
        if (pagination_style != "none") {
          if (controls.find(".slider_progress_bar").length > 0) {
            ...
            controls
              .find(".slider_pagination_button_wrap,.slider_pagination_bullet")
              .on("click", function (e) {
                var s = TRX_ADDONS_STORAGE["swipers"][slider_id];
                var slide_idx = jQuery("#" + slider_id)
                  .find(
              ...
              if (TRX_ADDONS_STORAGE["swipers"][slider_id]) {
                ...
                if (total > 0) {
                  if (pagination_style == "progressbar") {
                    ...
                    controls
                      .find(".slider_pagination_button")
                      .removeClass("slider_pagination_button_active")
                      .eq(current - 1)
                      .addClass("slider_pagination_button_active");
                  } else if (pagination_style == "bullets") {
                    controls
                      .find(".slider_pagination_bullet")
                      .removeClass(
                        "slider_pagination_bullet_active swiper-pagination-bullet-active"
                      )
                      .eq(current - 1)
                      .addClass(
                        "slider_pagination_bullet_active swiper-pagination-bullet-active"
      ...
      container.find(".slider_swiper:not(.inited)").each(function () {
        ...
        slider.find("img").each(function () {
          var $self = jQuery(this);
          if (!$self.get(0).complete && $self.attr("loading") != "lazy")
            loaded = false;
        });
        ...
        var is_controller =
          slider.parents(".sc_slider_controller").length > 0 || slave_id;
        ...
        slides_parallax =
          effect == "slide" && spv == 1
            ? Math.max(0, Math.min(1, slides_parallax || 0))
            : 0;
        if (
          direction == "vertical" &&
          slider.hasClass("slider_height_auto") &&
          slider.hasClass("slider_noresize")
        ) {
          var height = 0;
          slider.find(".swiper-slide").each(function (idx) {
            if (idx >= spv) return;
        ...
        var noswipe =
          slider.hasClass("slider_noswipe") ||
        ...
        TRX_ADDONS_STORAGE["swipers"][id] = new Swiper(
          "." + id,
          trx_addons_apply_filters(
            "trx_addons_filter_slider_init_args",
            {
              ...
              roundLengths:
                TRX_ADDONS_STORAGE["slider_round_lengths"] &&
                !is_controller &&
                spv == 1,
              effect: effect,
              parallax: { enabled: slides_parallax > 0 },
              swipeHandler: noswipe
                ? ".slider_controls_wrap,.slider_pagination_wrap"
                : null,
              pagination: {
                el: slider.hasClass("slider_pagination")
                  ? "#" +
                    cont_id +
                  ...
                  : null,
                clickable: slider.hasClass("slider_pagination")
                  ? "#" + cont_id + " .slider_pagination_wrap"
                  : false,
                type:
                  slider.hasClass("slider_pagination") &&
                  slider.data("pagination")
                    ? slider.data("pagination")
                    : "bullets",
                progressbarOpposite:
                  slider.data("pagination") == "progressbar" &&
                  ((slider.data("direction") == "vertical" &&
                    (slider.hasClass("slider_pagination_pos_bottom") ||
                      slider.hasClass(
                        "slider_pagination_pos_bottom_outside"
                      ))) ||
                    (slider.data("direction") == "horizontal" &&
                      (slider.hasClass("slider_pagination_pos_left") ||
                        slider.hasClass("slider_pagination_pos_right")))),
              },
              navigation: {
                nextEl: slider.hasClass("slider_controls")
                  ? "#" +
                    id +
                    ...
                    "~.slider_controls_wrap>.slider_next"
                  : null,
                prevEl: slider.hasClass("slider_controls")
                  ? "#" +
                    id +
              ...
              autoplay:
                slider.hasClass("slider_noautoplay") ||
                (typeof slider.data("autoplay") !== "undefined" &&
                  slider.data("autoplay") == 0)
                  ? false
                  : { delay: interval, disableOnInteraction: true },
              on: {
                slideChangeTransitionStart: function () {
                  ...
                  var slaves = is_controller
                    ...
                    : jQuery(
                        '[data-slider-id="' + id.replace("_sc_", "_") + '"]'
                      );
                  if (slaves.length === 0 && !is_controller) {
                    slaves = jQuery(
                      '[data-slider-id="' +
                        id.replace("_sc_", "_").replace("_swiper", "") +
                        '"]'
                    );
                    if (slaves.length === 0) {
                      slaves = jQuery(
                        '[data-slider-id="' +
                          id.replace("_sc_slider_swiper", "") +
                          '"]'
                      );
                    }
                  }
                  if (slaves.length > 0) {
                    slaves.each(function () {
                      var controlled_slider = jQuery(this).hasClass(
                        "slider_swiper"
                      )
                        ? jQuery(this)
                        : jQuery(this).find(".slider_swiper").eq(0);
                      ...
                      if (
                        ...
                        var slide_idx = controlled_slider
                          ...
                          .index();
                        TRX_ADDONS_STORAGE["swipers"][controlled_id].slideTo(
                          slide_idx
                        );
                      }
                    });
                ...
                slideChangeTransitionEnd: function () {
                  var swiper = this,
                    ...
                    slide_prev = jQuery(
                      swiper.slides[
                        swiper.lastOpenedIndex !== undefined
                          ? swiper.lastOpenedIndex
                          : swiper.previousIndex
                      ]
                    ),
                    slide_number = slide.data("slide-number");
                  swiper.lastOpenedIndex = swiper.activeIndex;
                  var titles = cont.find(
                    ".slider_titles_outside_wrap .slide_info"
                  );
                  if (titles.length > 0) {
                    titles.eq(slide_number).addClass("active").fadeIn(300);
                  }
                  var video = slide_prev.find(
                    ".trx_addons_video_player:not(.with_video_autoplay)"
                  );
                  if (video.length > 0) {
        ...
        if (slider.data("mouse-wheel") == 1) {
          ...
          if (slider_sticky_wrapper.length) {
            ...
            var init_sticky_observer = function () {
              var slider_sticky_wrapper_top =
                  slider_sticky_wrapper.css("top") || "0px",
                custom_top =
                  Math.abs(
                    parseFloat(slider_sticky_wrapper_top) -
                      trx_addons_fixed_rows_height()
                  ) > 1;
              trx_addons_sticky_observer_remove(id);
              trx_addons_sticky_observer_create(
                id,
                slider_sticky_wrapper,
                function (entry, is_sticky) {
                  if (is_sticky) {
                ...
                },
                {
                  rootMargin:
                    -(custom_top
                      ? parseFloat(slider_sticky_wrapper_top)
                      : trx_addons_fixed_rows_height()) + "px 0px 0px 0px",
                }
              );
            };
            init_sticky_observer();
            $document.on(
              "action.resize_trx_addons action.sc_layouts_row_fixed_on action.sc_layouts_row_fixed_off",
              trx_addons_debounce(init_sticky_observer, 500)
            );
          }
        }
        ...
        slider
          ...
          .animate({ opacity: 1 }, "fast", function () {
            slider.attr("data-busy", 0);
            trx_addons_set_controller_height(
              is_controller
                ? slider.parents("[data-slider-id]").eq(0).attr("id")
                : controller_id,
              is_controller
                ? jQuery(
                    "#" +
                      slider.parents("[data-slider-id]").eq(0).data("slider-id")
                  )
                : slider
            );
    ...
    if (window.ElastiStack) {
      container.find(".slider_elastistack:not(.inited)").each(function () {
        if (
          jQuery(this).parents("div:hidden,article:hidden").length > 0 ||
          typeof window.ElastiStack == "undefined"
        )
          return;
        var slider = jQuery(this);
        var id = slider.attr("id");
    ...
    container.find(".slider_container.inited").each(function () {
      ...
      var max_width = slider
        .closest(
          ".post_featured" +
            ...
            "," +
            trx_addons_apply_filters(
              "trx_addons_filter_content_class",
              ".content",
              "slider-resize"
            ) +
            "," +
      ...
      if (last_width === 0 || last_width != slider_width) {
        ...
        if (
          ...
          if (trx_addons_window_width() < 1440) {
            var grid_element = jQuery(
              ...
              grid_gap = grid_element.length
                ? parseInt(grid_element.css("padding-left"), 10)
                : 0;
            if (grid_gap >= 10) {
              gap = grid_gap * 2;
            }
        ...
        if (
          ...
          typeof TRX_ADDONS_STORAGE["swipers"][id] == "object" &&
          typeof TRX_ADDONS_STORAGE["swipers"][id].params == "object" &&
          typeof TRX_ADDONS_STORAGE["swipers"][id].params.slidesPerView !=
            "undefined"
        ) {
          if (
        ...
        if (!slider.hasClass("slider_noresize") || slider.height() === 0) {
          ...
          slider.height(
            direction == "vertical" ? height * spv + (spv - 1) * space : height
          );
          on_resize = true;
        ...
        if (
          on_resize &&
          (slider.hasClass("slider_swiper") ||
            slider.hasClass("slider_swiper_outer"))
        ) {
          if (
            typeof TRX_ADDONS_STORAGE["swipers"] != "undefined" &&
            typeof TRX_ADDONS_STORAGE["swipers"][id] == "object" &&
            typeof TRX_ADDONS_STORAGE["swipers"][id].resize == "object" &&
            typeof TRX_ADDONS_STORAGE["swipers"][id].resize.resizeHandler ==
              "function"
          ) {
  ...
  function trx_addons_set_controller_height(controller_id, slider, e) {
    if (
      !controller_id &&
      typeof TRX_ADDONS_STORAGE["pagebuilder_preview_mode"] != "undefined" &&
      TRX_ADDONS_STORAGE["pagebuilder_preview_mode"]
    ) {
      var slider_id = slider.attr("id").replace("_sc_slider", "");
      if (slider_id) {
        controller_id = jQuery('[data-slider-id="' + slider_id + '"]')
          .eq(0)
          .attr("id");
      }
    }
    ...
    if (
      ...
      var controller_slider = controller.hasClass("slider_container")
          ? controller
          : controller.find(".slider_container"),
        controller_slider_id = controller_slider.attr("id");
      var paddings = parseFloat(controller.css("paddingTop"));
      if (isNaN(paddings)) paddings = 0;
      var controller_spv = trx_addons_get_slides_per_view(controller);
      if (isNaN(controller_spv)) controller_spv = 1;
      controller_slider.height(
        Math.max(
          (slider.parent().hasClass("slider_outer")
            ? slider.parent().outerHeight()
            : slider.height()) -
            2 * paddings,
          controller_spv *
            trx_addons_apply_filters(
              "trx_addons_filter_slider_controller_min_height",
              80
            )
        )
      );
      if (
        controller_slider.hasClass("slider_swiper") ||
        controller_slider.hasClass("slider_swiper_outer")
      ) {
        if (
          typeof TRX_ADDONS_STORAGE["swipers"][controller_slider_id] ==
            "object" &&
          typeof TRX_ADDONS_STORAGE["swipers"][controller_slider_id].resize ==
            "object" &&
          typeof TRX_ADDONS_STORAGE["swipers"][controller_slider_id].resize
            .resizeHandler == "function"
        ) {
          TRX_ADDONS_STORAGE["swipers"][
            controller_slider_id
          ].resize.resizeHandler(e);
        }
      }
    }
  }
  $document.on("action.ready_trx_addons", function (e) {
    ...
    if (
      trx_addons_apply_filters(
        "trx_addons_filter_add_effect_to_swiper",
        true,
        "swap"
      )
    ) {
      const effectSwapModules = {
        setTranslate: function setTranslate(swiper) {
          ...
          for (var i = 0, length = slides.length; i < length; i += 1) {
            ...
            var place =
              distance < -1 * Math.max(1, spaceBetween)
                ? 1
            ...
            var rotateX = isHorizontal
              ? 0
              : allow
              ? -place * Math.min(rotate, rotate * delta)
              : 0;
            ...
            $slideEl[0].style.opacity =
              perView > 1 ? Math.max(0, 1 - delta) : 1;
        ...
        setTransition: function setTransition(
          ...
          const { transformEl } = swiper.params.swapEffect;
          ...
          if (swiper.params.slideShadows) {
            const $shadow = $transitionElements.find(
              ".swiper-slide-shadow-top, .swiper-slide-shadow-right, .swiper-slide-shadow-bottom, .swiper-slide-shadow-left"
            );
      ...
      if (
        typeof Swiper.prototype.modules != "undefined" &&
        typeof Swiper.prototype.modules["effect-fade"] != "undefined"
      ) {
        ...
        Swiper.__proto__.EffectSwap = {
          name: "effect-swap",
          params: { swapEffect: { rotate: 50, scale: 1 } },
          ...
          on: {
            beforeInit: function beforeInit() {
              ...
              swiper.classNames.push(
                swiper.params.containerModifierClass + "swap"
              );
              swiper.classNames.push(
        ...
        const EffectSwap = function (_ref) {
          ...
          extendParams({
            swapEffect: { rotate: 50, scale: 1, transformEl: null },
          });
          const setTranslate = () => {
...
jQuery(document).on("action.ready_trx_addons", function () {
    ...
    if ($sc_cart.length > 0) {
      ...
      $document.on("action.prepare_popup_elements", function (e, panel) {
        if (panel.hasClass("sc_layouts_cart_panel")) {
          $sc_cart
            .find('[data-panel-id="' + panel.attr("id") + '"]')
            .each(function () {
      ...
      if (
        trx_addons_apply_filters(
          "trx_addons_filter_sc_layouts_cart_button_hide_on_scroll",
          true
        )
      ) {
        $document.on("action.scroll_trx_addons", function () {
      ...
      jQuery(document.body).on(
        "wc_fragments_refreshed wc_fragments_loaded update_cart added_to_cart removed_from_cart",
        function (e) {
          jQuery(".widget_shopping_cart").each(function () {
            ...
            $items.text(
              items[0] +
                (items.length > 1
                  ? " " +
                    (cnt == 1 ? $items.data("item") : $items.data("items"))
                  : "")
            );
            jQuery(".sc_layouts_cart_items_short").text(items[0]);
            $sc_cart.data({ items: cnt ? cnt : 0, summa: total ? total : 0 });
      ...
      jQuery(document.body).on(
        ...
        function (e, data) {
          var items = $sc_cart
            .find(".sc_layouts_cart_items")
            .eq(0)
            .text()
            .split(" ", 2);
          items[0] = data.cart_quantity ? data.cart_quantity : data.quantity;
          jQuery(".sc_layouts_cart_items").text(
  ...
  function sc_layouts_cart_panel_open($sc_cart) {
    if (!$sc_cart.hasClass("sc_layouts_cart_opened")) {
      var link = $sc_cart
        .filter(":visible")
        .find(".sc_layouts_cart_icon,.sc_layouts_cart_details")
        .eq(0);
      if (link.length) {
        if (link.data("panel-id")) {
          link.trigger("click");
          ...
          setTimeout(function () {
            jQuery("#" + link.data("button-id")).toggleClass(
...
(function () {
    ...
    window.trx_addons_init_sfmenu = function (selector) {
      jQuery(selector)
        ...
        .each(function () {
          ...
          var sf_init = {
            ...
            onBeforeShow: function () {
              jQuery(this).each(function () {
                ...
                if (
                  ...
                  !menu_item.hasClass("layouts_inited") &&
                  menu_item.find(".slider_container").length > 0
                ) {
                  menu_item.addClass("sc_layouts_submenu_prepare");
                } else {
                  trx_addons_do_action(
                    "trx_addons_action_menu_on_before_show",
                    menu_item
                  );
            ...
            onShow: function () {
              jQuery(this).each(function () {
                ...
                if (menu_item.data("menu-state") != "before-show") {
                  trx_addons_do_action(
                    "trx_addons_action_menu_on_show",
                    menu_item
                  );
                  trx_addons_before_show_menu(menu_item);
                }
                ...
                trx_addons_do_action(
                  "trx_addons_action_menu_after_show",
                  menu_item
                );
            ...
            onBeforeHide: function () {
              jQuery(this).each(function () {
                ...
                if (menu_item.data("menu-state") == "show") {
                  ...
                  trx_addons_do_action(
                    "trx_addons_action_menu_on_before_hide",
                    menu_item
                  );
            ...
            onHide: function () {
              jQuery(this).each(function () {
                ...
                trx_addons_do_action(
                  "trx_addons_action_menu_on_hide",
                  menu_item
                );
          ...
          if (animation_in == "none") {
            sf_init.animation = { opacity: "show" };
          }
          if (animation_out == "none") {
            sf_init.animationOut = { opacity: "hide" };
          }
          $self.find(".sc_layouts_submenu_wrap").on("focusout", function (e) {
            if (
              e.currentTarget &&
              jQuery(e.currentTarget).hasClass("sc_layouts_submenu_wrap")
            ) {
              e.stopPropagation();
              return false;
            }
          ...
          $self
            ...
            .superfish(
              trx_addons_apply_filters(
                "trx_addons_filter_menu_init_args",
                sf_init
              )
            );
          function trx_addons_before_show_menu(menu_item) {
            var in_columns =
              menu_item.parents('li[class*="columns-"]').length > 0 &&
            ...
            if (!in_columns) {
              var window_width = jQuery(window).width(),
                ...
                page_wrap_width =
                  page_wrap.length > 0 ? page_wrap.width() : window_width,
                page_wrap_offset =
                  page_wrap.length > 0 ? page_wrap.offset().left : 0,
              ...
              if (menu_item.parents("ul").length > 1) {
                if (
                  (!rtl &&
                    ((par_offset + par_width + ul_width >
                      page_wrap_offset + page_wrap_width - 10 &&
                      par_offset - ul_width > page_wrap_offset) ||
                      (par_offset + par_width + ul_width > window_width &&
                        par_offset - ul_width > 0))) ||
                  (rtl &&
                    ((par_offset - ul_width < page_wrap_offset + 10 &&
                      par_offset + par_width + ul_width <
                        page_wrap_offset + page_wrap_width) ||
                      (par_offset - ul_width < 0 &&
                        par_offset + par_width + ul_width < window_width)))
                ) {
              ...
              if (menu_item.parents(".top_panel").length > 0) {
                ...
                if (!wide) {
                  ...
                  if (ul_pos === undefined) {
                    ul_pos = parseFloat(
                      menu_item.css(
                        menu_item.hasClass("submenu_left") ? "right" : "left"
                      )
                    );
                  }
                  if (isNaN(ul_pos)) {
                  ...
                  if (menu_item.hasClass("submenu_left")) {
                    if (ul_offset < 0) {
                      ...
                      menu_item.css({ right: ul_pos + ul_offset + "px" });
                    }
                  } else {
                    if (ul_offset + ul_width >= window_width) {
                      ...
                      menu_item.css({
                        left:
                          ul_pos - (ul_offset + ul_width - window_width) + "px",
                      });
                    }
                  }
                  var ul_height = menu_item.outerHeight(),
                    ...
                    row_offset = menu.length
                      ? menu.offset().top - jQuery(window).scrollTop()
                      : 0,
                    row_height = 0,
                    par_top = 0;
              ...
              var animated = false;
              trx_addons_do_action(
                "trx_addons_action_menu_before_animation_in",
                menu_item,
                animation_in,
                animation_out
              );
              if (animation_in != "none") {
                animated = trx_addons_apply_filters(
                ...
                if (!animated) {
                  if (
                    menu_item.hasClass("animated") &&
                    menu_item.hasClass(animation_out)
                  ) {
            ...
            if (menu_item.hasClass("sc_layouts_submenu")) {
              if (!menu_item.hasClass("layouts_inited")) {
                ...
                if (menu_item.find(".slider_container").length > 0) {
                  $document.on(
                    "action.slider_inited",
                    function (e, slider, id) {
                      trx_addons_before_show_menu(menu_item);
                      menu_item
          ...
          function trx_addons_before_hide_menu(menu_item) {
            ...
            var animated = false;
            trx_addons_do_action(
              "trx_addons_action_menu_before_animation_out",
              menu_item,
              animation_in,
              animation_out
            );
            if (animation_out != "none") {
              ...
              if (!animated) {
                if (menu_item.parents('[class*="columns-"]').length === 0) {
                  if (
                    menu_item.hasClass("animated") &&
                    menu_item.hasClass(animation_in)
                  ) {
          ...
          function trx_addons_stretch_submenu(menu_item) {
            ...
            if (
              ...
              !menu_item
                .parents(".sc_layouts_menu")
                .hasClass("sc_layouts_menu_dir_vertical") &&
              trx_addons_apply_filters(
                "trx_addons_filter_stretch_menu",
                menu_item.hasClass("sc_layouts_submenu") ||
                  menu_item.parent().attr("class").indexOf("columns-") != -1,
                menu_item
              )
              ...
              if (menu.length == 1) {
                var $body = jQuery("body"),
                  ...
                  stretch_to = trx_addons_apply_filters(
                    "trx_addons_filter_stretch_menu_to",
                  ...
                  content_wrap_selector = trx_addons_apply_filters(
                    "trx_addons_filter_stretch_menu_content_wrap_selector",
                    ".content_wrap",
                    menu_item
                  ),
                ...
                if (!content_wrap.length) {
                  $body.append(
                    trx_addons_apply_filters(
                      "trx_addons_filter_stretch_menu_content_wrap_html",
                  ...
                  content_wrap = jQuery(content_wrap_selector).eq(0);
                  if (!content_wrap.length) {
                    content_wrap = trx_addons_apply_filters(
                      "trx_addons_filter_stretch_menu_content_wrap",
                      content_wrap,
                      menu_item
                    );
                  }
                ...
                if (content_wrap.length == 1) {
                  var bw = $body.innerWidth(),
                    cw = trx_addons_apply_filters(
                      "trx_addons_filter_stretch_menu_content_wrap_width",
                      content_wrap.innerWidth(),
                      menu_item,
                      content_wrap
                    ),
                  ...
                  if (stretch_to == "window") {
                    menu_item
                      .data(
                        "reset-style",
                        ".elementor-section-boxed > .elementor-container,.e-con-boxed > .e-con-inner"
                      )
                      .find(
                        ".elementor-section-boxed > .elementor-container,.e-con-boxed > .e-con-inner"
                      )
                    ...
                    if (bg.length === 0) {
                      menu_item.append(
                        '<span class="sc_layouts_menu_stretch_bg"></span>'
                      );
  ...
  function trx_addons_menu_collapse() {
    ...
    jQuery(
      ".sc_layouts_menu:not(.sc_layouts_menu_no_collapse):not(.sc_layouts_menu_dir_vertical)"
    ).each(function () {
      var nav = jQuery(this);
      ...
      function check_menu_wrapper(item) {
        var allow_any_wrapper = trx_addons_apply_filters(
          "trx_addons_filter_menu_collapse_allow_any_wrapper",
          true
        );
      ...
      sc_layouts_item_parent
        .find(
          ">div" +
            (sc_layouts_item_parent.attr("class").indexOf("elementor-") >= 0
              ? ".elementor-element"
              : "")
        )
        .each(function () {
          ...
          if (check_item_delimiter($self)) {
            ...
            if ($self.find("#" + ul_id).length > 0) {
              ...
              w_siblings +=
                $self.outerWidth() +
                Math.ceil(parseFloat($self.css("marginLeft"))) +
                Math.ceil(parseFloat($self.css("marginRight")));
            }
          }
        });
      w_max -= w_siblings;
    ...
    jQuery(".search_wrap:not(.inited)").each(function () {
      ...
      search_wrap.find(".search_submit").on("click", function (e) {
        ...
        if (
          ...
          var duration = trx_addons_apply_filters(
            "trx_addons_filter_search_fullscreen_fade_duration",
            0
          );
...
jQuery(document).ready(function () {
  ...
  function update_jquery_links(e) {
    ...
    $reviews = jQuery(
      ".trx_addons_reviews_block_mark:not(.inited)" +
      ...
      .on("mousemove", function (e) {
        var mark_wrap = jQuery(this),
          ...
          x =
            Math.min(
              w,
              Math.max(0, Math.round(e.pageX - mark_wrap.offset().left))
            ) + 1;
        if (x <= w) {
          var pos = Math.round((x / w) * 100),
            mark_show = trx_addons_reviews_mark2show(
              pos,
              mark_max,
              mark_decimals
            );
          pos = trx_addons_reviews_mark2save(
            mark_show,
            mark_max,
            mark_decimals
          );
      ...
      .on("mouseleave", function (e) {
        var mark_wrap = jQuery(this),
          pos = Math.max(
            0,
            Number(
              mark_wrap.find('input[name="trx_addons_reviews_vote"]').val()
            )
          );
  ...
  function trx_addons_reviews_block_mark_init(e, container) {
    ...
    $reviews.each(function (idx) {
      ...
      if (
        item.hasClass("inited") ||
        item.parents("div:hidden,article:hidden").length > 0
      ) {
        return;
      }
