...
(function () {
  ...
  var $mouse_helper_targets, $mouse_helper_magnets;
  ...
  function update_jquery_links(e) {
    ...
    $mouse_helper_magnets = jQuery(
      '[data-mouse-helper-magnet]:not([data-mouse-helper-magnet="0"])'
    );
  }
  update_jquery_links();
    ...
    if (
      ...
      $document
        .on("mousemove pointermove", function (e) {
          ...
          if (
            mouse_helper_callback &&
            typeof window[mouse_helper_callback] == "function"
          ) {
            window[mouse_helper_callback](
              "mousemove",
              $mouse_helper,
              mouse_helper_target,
              e
            ...
            typeof window[
              "trx_addons_mouse_helper_callback_" + mouse_helper_action
            ] == "function"
          ) {
            window["trx_addons_mouse_helper_callback_" + mouse_helper_action](
              "mousemove",
              $mouse_helper,
              mouse_helper_target,
              e
          ...
          if (mouse_helper_action == "highlight" && mouse_helper_target) {
            var targetOffset = mouse_helper_target.offset(),
              ...
              size =
                parseFloat(
                  mouse_helper_target.css("background-size").split(" ")[0]
                ) / 2;
            mouse_helper_target.css(
              "background-position",
          ...
          if (
            ...
            TRX_ADDONS_STORAGE["mobile_breakpoint_mousehelper_off"]
          ) {
            if (mouse_helper_delay < 2) {
              ...
              $mouse_helper.css(
                "transform",
                "translate(" + destX[0] + "px," + destY[0] + "px)"
              );
            }
        ...
        .on("mouseenter", "[data-mouse-helper]", function (e) {
          ...
          if (
            ...
            // Centered
            var pos =
              (TRX_ADDONS_STORAGE["mouse_helper_centered"] > 0 &&
                ($self.data("mouse-helper-centered") === undefined ||
                  $self.data("mouse-helper-centered") === "")) ||
              $self.data("mouse-helper-centered") > 0 ||
              "";
            $mouse_helper.toggleClass(
            ...
            if (img) {
              $mouse_helper
                .eq(0)
                .find(".trx_addons_mouse_helper_image")
                .remove();
              $mouse_helper
                .eq(0)
                .append(
                  '<span class="trx_addons_mouse_helper_image" style="background-image:url(' +
                    img +
                    ');"></span>'
                )
            ...
            if (icon && icon != "none") {
              ...
              $mouse_helper
                .eq(0)
                .find(".trx_addons_mouse_helper_icon")
                .remove()
                .end()
                .append(
                  '<span class="trx_addons_mouse_helper_icon ' +
                    icon +
                    '"' +
                    ' style="' +
            ...
            if (text) {
              $mouse_helper
                .eq(0)
                .find(".trx_addons_mouse_helper_text")
                .remove()
                .end()
                .append(
                  '<span class="trx_addons_mouse_helper_text' +
                    (text_round ? " trx_addons_mouse_helper_text_round" : "") +
                    '"' +
                    (text_size
                    ...
                    ">" +
                    (text_round
                      ? trx_addons_wrap_chars(
                          text,
                          '<span class="trx_addons_mouse_helper_text_round_item">',
                          "</span>"
                        )
                      : text) +
                    "</span>"
              ...
              if (text_round) {
                ...
                setTimeout(function () {
                  var text_wrap = $mouse_helper
                      .eq(0)
                      .find(".trx_addons_mouse_helper_text"),
                    items = text_wrap.find(
                  ...
                  if (items.length > 0) {
                    ...
                    items.each(function (idx) {
                      if (hide) {
                        ...
                        if (a >= (rtl ? 310 : 330)) {
                          // If fill whole round - hide rest chars and replace its with '...'
                        ...
                        a +=
                          (2 *
                            Math.asin(
                              (items.eq(idx).width() + (rtl ? 3 : 3)) / (2 * r)
                            ) *
                            180) /
                          Math.PI;
                      }
            ...
            if (layout) {
              $mouse_helper
                .eq(0)
                .find(".trx_addons_mouse_helper_layout")
                .remove()
                .end()
                .append(
                  '<span class="trx_addons_mouse_helper_layout">' +
                    layout +
                    "</span>"
                )
            ...
            if (obj_type !== "") {
              $mouse_helper
                .eq(0)
                .addClass("trx_addons_mouse_helper_over_element_" + obj_type);
            }
            // Set 'active' and 'action' classes
            $mouse_helper.addClass(
              "trx_addons_mouse_helper_active trx_addons_mouse_helper_action_" +
                mouse_helper_action
            );
          }
        })
        ...
        .on("mouseleave", "[data-mouse-helper]", function (e) {
          ...
          $mouse_helper.removeClass(
            "trx_addons_mouse_helper_click" +
              " trx_addons_mouse_helper_action_" +
              mouse_helper_action
          );
          ...
          if (
            ...
            mouse_helper_timeout = setTimeout(
              function () {
                mouse_helper_reset();
              },
      ...
      var mouse_helper_reset = function () {
        ...
        setTimeout(
          ...
          $mouse_helper.eq(0).hasClass("trx_addons_mouse_helper_with_icon") ||
            ...
            $mouse_helper
              .eq(0)
              .hasClass("trx_addons_mouse_helper_with_image") ||
            $mouse_helper
              .eq(0)
              .hasClass("trx_addons_mouse_helper_with_layout") ||
            (mouse_helper_target &&
        ...
        $mouse_helper
          ...
          .attr(
            ...
            trx_addons_chg_class_by_prefix(
              $mouse_helper.attr("class"),
              "trx_addons_mouse_helper_over_element_",
              ""
            )
          );
        var $inner_elements = $mouse_helper
          .eq(0)
          .find(
            ".trx_addons_mouse_helper_image,.trx_addons_mouse_helper_text,.trx_addons_mouse_helper_icon,.trx_addons_mouse_helper_layout"
          );
        ...
        if (
          ...
          window[mouse_helper_callback](
            "reset",
            $mouse_helper,
            mouse_helper_last_target,
            null
          );
          mouse_helper_callback = "";
        } else if (
          typeof window[
            "trx_addons_mouse_helper_callback_" + mouse_helper_last_action
          ] == "function"
        ) {
          window[
            "trx_addons_mouse_helper_callback_" + mouse_helper_last_action
          ]("reset", $mouse_helper, mouse_helper_last_target, e);
        }
      };
          ...
          $mouse_helper.each(function (idx) {
            if (destX[idx] != mouseX || destY[idx] != mouseY) {
              if (
                $mouse_helper
                  .eq(idx)
                  .hasClass("trx_addons_mouse_helper_permanent") ||
                $mouse_helper.eq(idx).hasClass("trx_addons_mouse_helper_active")
              ) {
                var delay =
                  ($mouse_helper.eq(idx).data("delay") || 0) * 1 +
                  mouse_helper_delay * 1;
              ...
              $mouse_helper
                ...
                .css(
                  "transform",
                  "translate(" + destX[idx] + "px," + destY[idx] + "px)"
                );
            }
          });
        }
      ...
      function trx_addons_mouse_helper_check_magnets() {
        $mouse_helper_magnets.each(function () {
          var item = jQuery(this),
            inner = item.children(),
            koef = item.data("mouse-helper-magnet-velocity")
              ? item.data("mouse-helper-magnet-velocity")
              : 1,
          ...
          function control_item() {
            ...
            iy = off.top + ih / 2;
            near =
              Math.abs(ix - cx) < iw * koef && Math.abs(iy - cy) < ih * koef;
          ...
          function move_item() {
            ...
            function get_closest_position() {
              return {
                x:
                  Math.abs(cx - ix) < delta
                    ? cx - ix
                    : (delta * (cx - ix)) / Math.abs(cx - ix),
                y:
                  Math.abs(cy - iy) < delta
                    ? cy - iy
                    : (delta * (cy - iy)) / Math.abs(cy - iy),
              };
            }
          }
    ...
    jQuery("#toc_menu .toc_menu_item").each(function () {
      if (
        trx_addons_apply_filters(
          "trx_addons_filter_mouse_helper_on_toc",
          TRX_ADDONS_STORAGE["mouse_helper"] > 0
        )
      ) {
        var $self = jQuery(this),
          title =
            $self.attr("title") ||
            $self.find(".toc_menu_description_title").text();
        $self.attr({
          "data-mouse-helper": "hover",
          "data-mouse-helper-axis": "y",
          "data-mouse-helper-text":
            TRX_ADDONS_STORAGE["msg_mouse_helper_anchor"] +
            (title ? " " + title : ""),
        });
      }
    });
