...
(function () {
  ...
  window.addEventListener("load", function () {
    if (typeof trx_addons_apply_filters == "function") {
      globalCanvas = trx_addons_apply_filters(
        "trx_addons_filter_image_effects_use_global_canvas",
        globalCanvas
      );
  ...
  function create_planes() {
    ...
    var planeElements = document.querySelectorAll(
      '[class*="' +
        planeClassPrefix +
        '"]:not(.trx_addons_image_effects_inited)' +
        (jQuery("body").hasClass("allow_lazy_load") ? ".lazyload_inited" : "")
    );
    ...
    trx_addons_when_images_loaded(jQuery(planeElements), function () {
      ...
      for (var i = 0; i < planeElements.length; i++) {
        if (
          !firstLoad &&
          planeElements[i].closest(".elementor-section-stretched")
        )
          continue;
        if (!sliderInited && planeElements[i].closest(".slider-slide"))
          continue;
        total++;
        if (
          !planeElements[i].classList.contains(
            "trx_addons_image_effects_inited"
          ) &&
          jQuery(planeElements[i]).parents(":hidden").length === 0
        ) {
          effect = get_effect_name(planeElements[i]);
          if (
            effect &&
            typeof window["trx_addons_image_effects_callback_" + effect] ==
              "function"
          ) {
            window["trx_addons_image_effects_callback_" + effect](
              curtains,
              planeElements[i],
              i,
      ...
      if (total === planeElements.length) {
        ...
        $document.trigger("action.trx_addons_image_effects_inited", [
          planeElements,
        ]);
      }
    });
  ...
  $document.on(
    "action.before_remove_content action.deactivate_tab",
    function (e, cont) {
  ...
  $document.on(
    "action.after_add_content action.activate_tab",
    function (e, cont) {
      cont.find(".trx_addons_image_effects_inited").each(function () {
        jQuery(this)
          .removeClass("trx_addons_image_effects_inited")
          .find('[id^="trx_addons_image_effects_canvas_"]')
          .remove()
          .end()
          .find(".trx_addons_image_effects_holder")
          .removeClass("trx_addons_image_effects_holder")
          .end();
      });
    }
  );
  ...
  function get_mouse_position_from_event(e) {
    ...
    // touch event
    if (e.targetTouches) {
      mouse.x = globalCanvas
        ? e.targetTouches[0].clientX
        : e.targetTouches[0].layerX;
      mouse.y = globalCanvas
  ...
  function mouse_to_plane_coords(plane, mpos) {
    var w = plane.htmlElement.clientWidth,
      cw = w / 2,
      h = plane.htmlElement.clientHeight,
      ch = h / 2;
    return {
      x: (mpos.x - cw) / cw,
      y: -(mpos.y - ch) / ch,
  ...
  window.trx_addons_image_effects_callback_waves = function (
    curtains_global,
    elm,
    index,
    total
    ...
    var paddingOnHover =
      typeof trx_addons_apply_filters == "function"
    ...
    var effectStrength = Math.max(
      5.0,
      Math.min(50.0, elm.getAttribute("data-image-effect-strength") || 30.0)
    );

    var plane = null,
      parent = null,
      img_all = elm.querySelectorAll(
        'img:not([class*="trx_addons_image_effects_"])'
      ),
      img =
        img_all.length > 1
          ? elm.querySelector(
              'img:not([class*="avatar"]):not([class*="trx_addons_extended_taxonomy_img"])'
            )
        ...
        plane.tweenScale = trx_addons_tween_value({
          ...
          callbacks: {
            onUpdate: function (value) {
              ...
              if (scaleOnHover) {
                plane.textures &&
                  plane.textures[0].setScale(1 + value / 12, 1 + value / 12);
              }
            },
            onComplete: function () {
              trx_addons_tween_stop(plane.tweenScale);
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_waves2 = function (
    curtains_global,
    elm,
    index,
    total
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_ripple = function (
    curtains_global,
    elm,
    index,
    total
    ...
    var wavesDirection = Math.max(
      0.0,
      Math.min(1.0, elm.getAttribute("data-image-effect-waves-direction"))
    );
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_ripple2 = function (
    curtains_global,
    elm,
    index,
    total
    ...
    if (img) {
      ...
      if (displacement_url) {
        ...
        displacement_img.classList.add(
          "trx_addons_image_effects_ripple_displacement"
        );
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_smudge = function (
    curtains_global,
    elm,
    index,
    total
    ...
    function get_params(elm, img, parent) {
      ...
      return {
        ...
        uniforms: {
          ...
          padding: {
            // paddings around image on hover
            name: "uPadding",
            type: "1f",
            value: 0,
          },
      ...
      plane
        ...
        .onRender(function () {
          ...
          if (
            !permanentDrawing &&
            !enableDrawing &&
            Math.abs(deltas.applied - deltas.max) < 0.001
          ) {
            curtains.disableDrawing();
          }
        })
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_tint = function (
    curtains_global,
    elm,
    index,
    total
    ...
    var scaleOnHover = elm.getAttribute("data-image-effect-scale") > 0,
      scaleFactor = 0,
      tintColor =
        elm.getAttribute("data-image-effect-tint-color") ||
        TRX_ADDONS_STORAGE["theme_accent_color"];
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_swap = function (
    curtains_global,
    elm,
    index,
    total
    ...
    if (img) {
      ...
      if (displacement_url) {
        ...
        displacement_img.classList.add(
          "trx_addons_image_effects_swap_displacement"
        );
        parent.appendChild(displacement_img);
        $document.on("action.after_add_content", function (e, cont) {
    ...
    function handle_plane(plane) {
      plane
        ...
        .onRender(function () {
          plane.uniforms.time.value =
            activeImage == 1
              ? Math.min(60, plane.uniforms.time.value + 1)
              : Math.max(0, plane.uniforms.time.value - 1);
        })
        .onAfterResize(function () {
          //					console.log('Plane '+index+' of '+total+' is resized');
        });
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_fish = function (
    curtains_global,
    elm,
    index,
    total
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_blot = function (
    curtains_global,
    elm,
    index,
    total
  ...
  // Create plane.
  // Attention! All callbacks must be in a global scope!
  window.trx_addons_image_effects_callback_scroller = function (
    curtains_global,
    elm,
    index,
    total
    ...
    var effectStrength =
        ...
        Math.max(
          5.0,
          Math.min(50.0, elm.getAttribute("data-image-effect-strength") || 30)
        ),
      effectDirection =
        elm.getAttribute("data-image-effect-waves-direction") || 0;
      ...
      return {
        ...
        uniforms: {
          padding: {
            name: "uPadding",
            type: "1f",
            value: trx_addons_apply_filters(
              "trx_addons_filter_image_effects_padding",
              0.05,
              "scroller"
            ),
      ...
      plane
        .onReady(function () {
          ...
          var scrollBusy = false,
            clear_scroll_busy = trx_addons_throttle(
              function () {
                scrollBusy = false;
              },
              300,
